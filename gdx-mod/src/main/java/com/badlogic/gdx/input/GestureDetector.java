...
/**
 * {@link InputProcessor} implementation that detects gestures (tap, long press,
 * fling, pan, zoom, pinch) and hands them to a {@link GestureListener}.
 * 
 * @author mzechner
 */
public class GestureDetector extends InputAdapter {
	...
	/**
	 * @param halfTapSquareSize half width in pixels of the square around an initial
	 *                          touch event, see
	 ...
	 * @param longPressDuration time in seconds that must pass for the detector to
	 *                          fire a
	 *                          {@link GestureListener#longPress(float, float)}
	 *                          event.
	 * @param maxFlingDelay     time in seconds the finger must have been dragged
	...
	/**
	 * Register an instance of this class with a {@link GestureDetector} to receive
	 * gestures such as taps, long presses, flings, panning or pinch zooming. Each
	 * method returns a boolean indicating if the event should be handed to the next
	...
	public static interface GestureListener {
		...
		/**
		 * Called when a tap occured. A tap happens if a touch went down on the screen
		 * and was lifted again without moving outside of the tap square. The tap square
		 * is a rectangular area around the initial touch position as specified on
		 * construction time of the {@link GestureDetector}.
		 * 
		...
		/**
		 * Called when the user performs a pinch zoom gesture. The original distance is
		 * the distance in pixels when the gesture started.
		 * 
