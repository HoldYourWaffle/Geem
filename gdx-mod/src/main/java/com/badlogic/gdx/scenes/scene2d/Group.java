...
public class Group extends Actor implements Cullable {
	...
	/**
	 * Draws all children. {@link #applyTransform(Batch, Matrix4)} should be called
	 * before and {@link #resetTransform(Batch)} after this method if
	 * {@link #setTransform(boolean) transform} is true. If
	 * {@link #setTransform(boolean) transform} is false these methods don't need to
	 * be called, children positions are temporarily offset by the group position
	 * when drawn. This method avoids drawing children completely outside the
	 * {@link #setCullingArea(Rectangle) culling area}, if set.
	 */
	protected void drawChildren(Batch batch, float a) {
		a *= this.color.a;
	...
	/**
	 * Draws this actor's debug lines if {@link #getDebug()} is true and, regardless
	 * of {@link #getDebug()}, calls {@link Actor#drawDebug(ShapeRenderer)} on each
	 * child.
	 */
	@Override
	 ...
	 * {@link #computeTransform()}. Note this causes the shape renderer to be
	 * flushed. {@link #resetTransform(ShapeRenderer)} will restore the transform to
	 ...
	 * {@link #applyTransform(Batch, Matrix4)}. Note this causes the shape renderer
	 * to be flushed.
	 */
	...
	/**
	 * Removes an actor from this group. If the actor will not be used again and has
	 * actions, they should be {@link Actor#clearActions() cleared} so the actions
	 * will be returned to their {@link Action#setPool(com.badlogic.gdx.utils.Pool)
	 * pool}, if any. This is not done automatically.
	 * 
	...
	/**
	 * When true (the default), the Batch is transformed so children are drawn in
	 * their parent's coordinate system. This has a performance impact because
	 * {@link Batch#flush()} must be done before and after the transform. If the
	 * actors in a group are not rotated or scaled, then the transform for the group
	 * can be set to false. In this case, each child's position will be offset by
	 * the group's position for drawing, causing the children to appear in the
