...
/**
 * A progress bar is a widget that visually displays the progress of some
 * activity or a value within given range. The progress bar has a range (min,
 * max) and a stepping between each value it represents. The percentage of
 * completeness typically starts out as an empty progress bar and gradually
 * becomes filled in as the task or variable value progresses.
 * <p>
 * {@link ChangeEvent} is fired when the progress bar knob is moved. Cancelling
 * the event will move the knob to where it was previously.
 * <p>
 * For a horizontal progress bar, its preferred height is determined by the
 * larger of the knob and background, and the preferred width is 140, a
 * relatively arbitrary size. These parameters are reversed for a vertical
 * progress bar.
 * 
 * @author mzechner
 * @author Nathan Sweet
...
public class ProgressBar extends Widget implements Disableable {
	 ...
	 * prefWidth parameter, and its height is determined by the maximum of the
	 * height of either the progress bar {@link NinePatch} or progress bar handle
	 * {@link TextureRegion}. The min and max values determine the range the values
	 * of this progress bar can take on, the stepSize parameter specifies the
	...
	/**
	 * Returns the progress bar's style. Modifying the returned style may not have
	 * an effect until {@link #setStyle(ProgressBarStyle)} is called.
	 */
	...
	public void draw(Batch batch, float a) {
		...
		final Drawable bg = (disabled && style.disabledBackground != null) ? style.disabledBackground
				: style.background;
		final Drawable knobBefore = (disabled && style.disabledKnobBefore != null) ? style.disabledKnobBefore
				: style.knobBefore;
		final Drawable knobAfter = (disabled && style.disabledKnobAfter != null) ? style.disabledKnobAfter
				: style.knobAfter;
		...
		if (vertical) {
			...
			if (bg != null) {
				if (round)
					bg.draw(batch, Math.round(x + (width - bg.getMinWidth()) * 0.5f), y, Math.round(bg.getMinWidth()),
							height);
				else
			...
			if (knobBefore != null) {
				...
				if (bg != null)
					offset = bgTopHeight;
				if (round)
					knobBefore.draw(batch, Math.round(x + (width - knobBefore.getMinWidth()) * 0.5f),
							Math.round(y + offset), Math.round(knobBefore.getMinWidth()),
							Math.round(position + knobHeightHalf));
				else
					knobBefore.draw(batch, x + (width - knobBefore.getMinWidth()) * 0.5f, y + offset,
							knobBefore.getMinWidth(), position + knobHeightHalf);
			}
			if (knobAfter != null) {
				if (round)
			...
			if (knob != null) {
				if (round)
					knob.draw(batch, Math.round(x + (width - knobWidth) * 0.5f), Math.round(y + position),
							Math.round(knobWidth), Math.round(knobHeight));
				else
			...
			if (bg != null) {
				if (round)
					bg.draw(batch, x, Math.round(y + (height - bg.getMinHeight()) * 0.5f), width,
							Math.round(bg.getMinHeight()));
				else
			...
			if (knobBefore != null) {
				...
				if (bg != null)
					offset = bgLeftWidth;
				if (round)
					knobBefore.draw(batch, Math.round(x + offset),
			...
			if (knob != null) {
				if (round)
					knob.draw(batch, Math.round(x + position), Math.round(y + (height - knobHeight) * 0.5f),
							Math.round(knobWidth), Math.round(knobHeight));
				else
	...
	/**
	 * Sets the progress bar position, rounded to the nearest step size and clamped
	 * to the minimum and maximum values. {@link #clamp(float)} can be overridden to
	 * allow values outside of the progress bar's min/max range.
	 * 
	 * @return false if the value was not changed because the progress bar already
	 *         had the value or it was canceled by a listener.
	 */
	...
	/**
	 * If true (the default), inner Drawable positions and sizes are rounded to
	 * integers.
	 */
