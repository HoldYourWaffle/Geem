...
abstract public class Action implements Poolable {
	 ...
	 * automatically when an action is added to an actor. This method is also called
	 * with null when an action is removed from an actor.
	 * <p>
	 * When set to null, if the action has a {@link #setPool(Pool) pool} then the
	 * action is {@link Pool#free(Object) returned} to the pool (which calls
	 * {@link #reset()}) and the pool is set to null. If the action does not have a
	 * pool, {@link #reset()} is not called.
	 * <p>
	 * This method is not typically a good place for an action subclass to query the
	 * actor's state because the action may not be executed for some time, eg it may
	 * be {@link DelayAction delayed}. The actor's state is best queried in the
	 ...
	 * every usage of this action or computed during the action does not need to be
	 * reset.
	 * <p>
