...
/**
 * A container that contains two widgets and is divided either horizontally or
 * vertically. The user may resize the widgets. The child widgets are always
 * sized to fill their side of the SplitPane.
 * <p>
 * Minimum and maximum split amounts can be set to limit the motion of the
 * resizing handle. The handle position is also prevented from shrinking the
 * children below their minimum sizes. If these limits over-constrain the
 * handle, it will be locked and placed at an averaged location, resulting in
 * cropped children. The minimum child size can be ignored (allowing dynamic
 * cropping) by wrapping the child in a {@linkplain Container} with a minimum
 * size of 0 and {@linkplain Container#fill() fill()} set, or by overriding
 * {@link #clampSplitAmount()}.
 * <p>
 * The preferred size of a SplitPane is that of the child widgets and the size
 * of the {@link SplitPaneStyle#handle}. The widgets are sized depending on the
 * SplitPane size and the {@link #setSplitAmount(float) split position}.
 * 
 * @author mzechner
 * @author Nathan Sweet
 */
