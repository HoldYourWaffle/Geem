...
/**
 * A 2D scene graph containing hierarchies of {@link Actor actors}. Stage
 ...
 * and sets up the camera used to convert between stage coordinates and screen
 * coordinates.
 * <p>
 * A stage must receive input events so it can distribute them to actors. This
 * is typically done by passing the stage to
 * {@link Input#setInputProcessor(com.badlogic.gdx.InputProcessor)
 * Gdx.input.setInputProcessor}. An {@link InputMultiplexer} may be used to
 * handle input events before or after the stage does. If an actor handles an
 * event by returning true from the input method, then the stage's input method
 * will also return true, causing subsequent InputProcessors to not receive the
 * event.
 * <p>
 * The Stage and its constituents (like Actors and Listeners) are not
 * thread-safe and should only be updated and queried from a single thread
 * (presumably the main render thread). Methods should be reentrant, so you can
 * update Actors and Stages from within callbacks and handlers.
 * 
 * @author mzechner
 * @author Nathan Sweet
 */
public class Stage extends InputAdapter implements Disposable {
	 ...
	 * {@link Scaling#stretch}. The stage will use its own {@link Batch} which will
	 * be disposed when the stage is disposed.
	 */
	public Stage() {
	 ...
	 * avoid creating a new batch (which can be somewhat slow) if multiple stages
	 * are used during an application's life time.
	 * 
	...
	/**
	 * Calls {@link #act(float)} with {@link Graphics#getDeltaTime()}, limited to a
	 * minimum of 30fps.
	 */
	public void act() {
	...
	public void act(float delta) {
		// Update over actors. Done in act() because actors may change position, which
		// can fire enter/exit without an input event.
	...
	/**
	 * Applies a touch down event to the stage and returns true if an actor in the
	 * scene {@link Event#handle() handled} the event.
	 */
	@Override
	...
	/**
	 * Applies a touch moved event to the stage and returns true if an actor in the
	 * scene {@link Event#handle() handled} the event. Only {@link InputListener
	...
	/**
	 * Applies a mouse moved event to the stage and returns true if an actor in the
	 * scene {@link Event#handle() handled} the event. This event only occurs on the
	 * desktop.
	 */
	@Override
	...
	/**
	 * Applies a mouse scroll event to the stage and returns true if an actor in the
	 * scene {@link Event#handle() handled} the event. This event only occurs on the
	 * desktop.
	 */
	@Override
	 ...
	 * the specified pointer and button. The actor will be used as the
	 * {@link Event#getListenerActor() listener actor} and {@link Event#getTarget()
	 * target}.
	 */
	 ...
	 * events for the specified pointer and button. Note the listener may never
	 * receive a touchUp event if this method is used.
	 */
	...
	public void cancelTouchFocus(Actor actor) {
		...
		// Cancel all current touch focuses for the specified listener, allowing for
		// concurrent modification, and never cancel the
		// same focus twice.
		SnapshotArray<TouchFocus> touchFocuses = this.touchFocuses;
		...
		for (int i = 0, n = touchFocuses.size; i < n; i++) {
			...
			// Cannot return TouchFocus to pool, as it may still be in use (eg if
			// cancelTouchFocus is called from touchDragged).
		}
		touchFocuses.end();
	 ...
	 * touchDragged and touchUp events and removes their touch focus. This method
	 * removes all touch focus listeners, but sends a touchUp event so that the
	 * state of the listeners remains consistent (listeners typically expect to
	 * receive touchUp eventually). The location of the touchUp is
	 * {@link Integer#MIN_VALUE}. Listeners can use
	...
	public void cancelTouchFocusExcept(EventListener exceptListener, Actor exceptActor) {
		...
		// Cancel all current touch focuses except for the specified listener, allowing
		// for concurrent modification, and never
	...
	/**
	 * Removes the touch, keyboard, and scroll focus for the specified actor and any
	 * descendants.
	 */
	 ...
	 * @return true if the unfocus and focus events were not cancelled by a
	 *         {@link FocusListener}.
	 */
	...
	/**
	 * Replaces the root group. Usually this is not necessary but a subclass may be
	 * desired in some cases, eg being notified of {@link Group#childrenChanged()}.
	 */
	...
	/**
	 * Returns the {@link Actor} at the specified location in stage coordinates. Hit
	 * testing is performed in the order the actors were inserted into the stage,
	 * last inserted actors being tested first. To get stage coordinates from screen
	 * coordinates, use {@link #screenToStageCoordinates(Vector2)}.
	 * 
	 * @param touchable If true, the hit detection will respect the
	 *                  {@link Actor#setTouchable(Touchable) touchability}.
	 ...
	 * 
	 * @param screenCoords Input screen coordinates and output for resulting stage
	 *                     coordinates.
	 */
	 ...
	 * 
	 * @param stageCoords Input stage coordinates and output for resulting screen
	 *                    coordinates.
	 */
	 ...
	 * them. The transform matrix is typically obtained from
	 * {@link Batch#getTransformMatrix()} during {@link Actor#draw(Batch, float)}.
	 * 
	 ...
	 * 
	 * @see ScissorStack#calculateScissors(Camera, float, float, float, float,
	...
	/**
	 * If true, any actions executed during a call to {@link #act()}) will result in
	 * a call to {@link Graphics#requestRendering()} . Widgets that animate or
	 * otherwise require additional rendering may check this setting before calling
	...
	/**
	 * If true, debug lines are shown for actors even when {@link Actor#isVisible()}
	 * is false.
	 */
	...
	/**
	 * If true, debug is enabled only for the actor under the mouse. Can be combined
	 ...
	 * Can be combined with {@link #setDebugAll(boolean)}.
	 */
	...
	/**
	 * If not {@link Debug#none}, debug is enabled only for the first ascendant of
	 * the actor under the mouse that is a table. Can be combined with
	...
	/**
	 * If true, debug is enabled only for the first ascendant of the actor under the
