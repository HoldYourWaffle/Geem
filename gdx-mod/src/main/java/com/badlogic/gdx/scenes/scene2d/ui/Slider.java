...
/**
 * A slider is a horizontal indicator that allows a user to set a value. The
 * slider has a range (min, max) and a stepping between each value the slider
 * represents.
 * <p>
 * {@link ChangeEvent} is fired when the slider knob is moved. Canceling the
 * event will move the knob to where it was previously.
 * <p>
...
public class Slider extends ProgressBar {
	 ...
	 * parameter, its height is determined by the maximum of the height of either
	 * the slider {@link NinePatch} or slider handle {@link TextureRegion}. The min
	 * and max values determine the range the values of this slider can take on, the
	 * stepSize parameter specifies the distance between individual values. E.g. min
	 * could be 4, max could be 10 and stepSize could be 0.2, giving you a total of
	 * 30 values, 4.0 4.2, 4.4 and so on.
	 * 
