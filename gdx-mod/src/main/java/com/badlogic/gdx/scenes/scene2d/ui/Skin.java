 ...
 * fonts, colors, etc). Resources are named and can be looked up by name and
 * type. Resources can be described in JSON. Skin provides useful conversions,
 * such as allowing access to regions in the atlas as ninepatches, sprites,
 * drawables, etc. The get* methods return an instance of the object in the
 * skin. The new* methods return a copy of an instance in the skin.
 * <p>
 * See the
 * <a href="https://github.com/libgdx/libgdx/wiki/Skin">documentation</a> for
 * more.
 * 
 * @author Nathan Sweet
 */
public class Skin implements Disposable {
	...
	/**
	 * Creates a skin containing the resources in the specified skin JSON file. If a
	 * file in the same directory with a ".atlas" extension exists, it is loaded as
	 * a {@link TextureAtlas} and the texture regions added to the skin. The atlas
	 * is automatically disposed when the skin is disposed.
	 */
	 ...
	 * the texture regions from the specified atlas. The atlas is automatically
	...
	/**
	 * Creates a skin containing the texture regions from the specified atlas. The
	 ...
	 * exists with the name, a region is created from the texture and stored in the
	 * skin.
	 */
	...
	/**
	 * @return an array with the {@link TextureRegion} that have an index != -1, or
	 ...
	 * region exists with the name, a tiled drawable is created from the region and
	 * stored in the skin.
	 */
	 ...
	 * with the name, a ninepatch is created from the region and stored in the skin.
	 * If the region is an {@link AtlasRegion} then the {@link AtlasRegion#splits}
	 ...
	 * the name, a sprite is created from the region and stored in the skin. If the
	 * region is an {@link AtlasRegion} then an {@link AtlasSprite} is used if the
	 * region has been whitespace stripped or packed rotated 90 degrees.
	 */
	 ...
	 * ninepatch, or sprite exists with the name, then the appropriate drawable is
	 * created and stored in the skin.
	 */
	 ...
	 * skin. This compares potentially every style object in the skin of the same
	 * type as the specified style, which may be a somewhat expensive operation.
	 */
	...
	/**
	 * Disposes the {@link TextureAtlas} and all {@link Disposable} resources in the
	 * skin.
	 */
	@Override
	public void dispose() {
	...
	/**
	 * Returns a map of {@link Json#addClassTag(String, Class) class tags} that will
	 * be used when loading skin JSON. The map can be modified before calling
	 * {@link #load(FileHandle)}. By default the map is populated with the simple
	 * class names of libGDX classes commonly used in skins.
	 */
