 ...
 * the Stage (see {@link #handle()}). The default {@link Actor#fire(Event)} will
 * mark events handled if an {@link EventListener} returns true.
 * <p>
 * A cancelled event will be stopped and handled. Additionally, many actors will
 * undo the side-effects of a canceled event. (See {@link #cancel()}.)
 * 
 * @see InputEvent
...
public class Event implements Poolable {
	 ...
	 * will eat the event so it is not passed on to the application under the stage.
	 */
	public void handle() {
		handled = true;
	}
	 ...
	 * the event is for a checkbox being checked, cancelling the event could uncheck
	 * the checkbox.
	 */
	public void cancel() {
		cancelled = true;
