 ...
 * scale, rotation, Z index, and color. The position corresponds to the
 * unrotated, unscaled bottom left corner of the actor. The position is relative
 * to the actor's parent. The origin is relative to the position and is used for
 * scale and rotation.
 * <p>
 * An actor has a list of in progress {@link Action actions} that are applied to
 * the actor (often over time). These are generally used to change the
 * presentation of the actor (moving it, resizing it, etc). See
 * {@link #act(float)}, {@link Action} and its many subclasses.
 * <p>
 * An actor has two kinds of listeners associated with it: "capture" and
 * regular. The listeners are notified of events the actor or its children
 * receive. The regular listeners are designed to allow an actor to respond to
 * events that have been delivered. The capture listeners are designed to allow
 * a parent or container actor to handle events before child actors. See
 * {@link #fire} for more details.
 * <p>
 * An {@link InputListener} can receive all the basic input events. More complex
 * listeners (like {@link ClickListener} and {@link ActorGestureListener}) can
 * listen for and combine primitive events and recognize complex interactions
 * like multi-touch or pinch.
 * 
 * @author mzechner
 * @author Nathan Sweet
 */
public class Actor {
	 ...
	 * {@link Batch#begin()} has already been called on the batch. If
	 * {@link Batch#end()} is called to draw without the batch then
	 * {@link Batch#begin()} must be called before the method returns.
	 * <p>
	 * The default implementation does nothing.
	 * 
	 * @param a The parent alpha, to be multiplied with this actor's alpha, allowing
	 ...
	 * <p>
	 * The default implementation calls {@link Action#act(float)} on each action and
	 * removes actions that are complete.
	 * 
	 ...
	 * actor is not in the stage, the stage must be set before calling this method.
	 * <p>
	 * Events are fired in 2 phases:
	 * <ol>
	 * <li>The first phase (the "capture" phase) notifies listeners on each actor
	 * starting at the root and propagating downward to (and including) this
	 * actor.</li>
	 * <li>The second phase notifies listeners on each actor starting at this actor
	 * and, if {@link Event#getBubbles()} is true, propagating upward to the
	 * root.</li>
	 * </ol>
	 * If the event is {@link Event#stop() stopped} at any time, it will not
	 * propagate to the next actor.
	 * 
	 ...
	 * if no actor was hit. The point is specified in the actor's local coordinate
	 * system (0,0 is the bottom left of the actor and width,height is the upper
	 * right).
	 * <p>
	 * This method is used to delegate touchDown, mouse, and enter/exit events. If
	 * this method returns null, those events will not occur on this Actor.
	 * <p>
