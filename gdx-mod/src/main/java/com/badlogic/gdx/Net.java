 ...
 * and post requests, and TCP server/client socket communication.
 * </p>
 * 
 ...
 * </p>
 * 
 * To create a TCP client socket to communicate with a remote TCP server, invoke
 * the {@link #newClientSocket(Protocol, String, int, SocketHints)} method. The
 * returned {@link Socket} offers an {@link InputStream} and
 * {@link OutputStream} to communicate with the end point.
 * </p>
 * 
 * To create a TCP server socket that waits for incoming connections, invoke the
 * {@link #newServerSocket(Protocol, int, ServerSocketHints)} method. The
 * returned {@link ServerSocket} offers an
 * {@link ServerSocket#accept(SocketHints options)} method that waits for an
 * incoming connection.
 * 
 * @author mzechner
 * @author noblemaster
...
public interface Net {
	...
	public static class HttpRequest implements Poolable {
		 ...
		 * {@link HttpMethods}.
		 * 
		 * @param httpMethod This is the HTTP method for the request, see
		 *                   {@link HttpMethods}
		 */
		 ...
		 * 
		 * @param content A string encoded in the corresponding Content-Encoding set in
		 *                the headers, with the data to send with the HTTP request. For
		 *                example, in case of HTTP GET, the content is used as the query
		 *                string of the GET while on a HTTP POST it is used to send the
		 *                POST data.
		 */
		 ...
		 * 
		 * @param timeOut the number of milliseconds to wait before giving up, 0 or
		 ...
		 * @exception IllegalArgumentException if redirection is disabled on the GWT
		 *                                     backend.
		 */
	...
	public static interface HttpResponseListener {
		 ...
		 * 
		 * @param t If the HTTP request failed because an Exception, t encapsulates it
	 ...
	 * @param httpResponseListener The {@link HttpResponseListener} to call once the
	 *                             HTTP response is ready to be processed. Could be
	 ...
	 * @param hints    additional {@link ServerSocketHints} used to create the
	 *                 socket. Input null to use the default setting provided by the
	 *                 system.
	 ...
	 *              Input null to use the default setting provided by the system.
	 * @return the {@link ServerSocket}
	 * @throws GdxRuntimeException in case the socket couldn't be opened
	 */
	 ...
	 * @param port  the port
	 * @param hints additional {@link SocketHints} used to create the socket. Input
	 *              null to use the default setting provided by the system.
	 ...
	 * URIs of the specified type is invoked. The application is determined from the
	 * protocol and path of the URI. A best effort is made to open the given URI;
	 * however, since external applications are involved, no guarantee can be made
	 * as to whether the URI was actually opened. If it is known that the URI was
	 * not opened, false will be returned; otherwise, true will be returned.
	 * 
