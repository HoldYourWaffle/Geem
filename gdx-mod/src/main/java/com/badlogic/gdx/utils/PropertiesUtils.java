...
/**
 * {@code PropertiesUtils} is a helper class that allows you to load and store
 * key/value pairs of an {@code ObjectMap<String,String>} with the same
...
public final class PropertiesUtils {
	...
	public static void load(ObjectMap<String, String> properties, Reader reader) throws IOException {
		...
		while (true) {
			...
			if (mode == SLASH) {
				...
				// if (Character.isWhitespace(nextChar)) { <-- not supported by GWT; replaced
				// with isSpace.
	 ...
	 * <code>java.util.Properties</code>.
	 * <p>
	 * Every entry in the <code>ObjectMap</code> is written out, one per line. For
	 * each entry the key string is written, then an ASCII <code>=</code>, then the
	 * associated element string. For the key, all space characters are written with
	 * a preceding <code>\</code> character. For the element, leading space
	 * characters, but not embedded or trailing space characters, are written with a
	 * preceding <code>\</code> character. The key and element characters
	 * <code>#</code>, <code>!</code>, <code>=</code>, and <code>:</code> are
	 * written with a preceding backslash to ensure that they are properly loaded.
	 * <p>
