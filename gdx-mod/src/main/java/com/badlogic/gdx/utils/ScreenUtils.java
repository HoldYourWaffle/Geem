...
public final class ScreenUtils {
	 ...
	 * always has {@link MathUtils#nextPowerOfTwo} dimensions and RGBA8888
	 * {@link Format}. It can be accessed via {@link TextureRegion#getTexture}. The
	 * texture is not managed and has to be reloaded manually on a context loss. The
	 * returned TextureRegion is flipped along the Y axis by default.
	 */
	 ...
	 * base {@link Texture} always has {@link MathUtils#nextPowerOfTwo} dimensions
	 * and RGBA8888 {@link Format}. It can be accessed via
	 * {@link TextureRegion#getTexture}. This texture is not managed and has to be
	 * reloaded manually on a context loss. If the width and height specified are
	 * larger than the framebuffer dimensions, the Texture will be padded
	 * accordingly. Pixels that fall outside of the current screen will have RGBA
	 * values of 0.
	 * 
	 ...
	 * data. Because of differences in screen and image origins the framebuffer
	 * contents should be flipped along the Y axis if you intend save them to disk
	 * as a bitmap. Flipping is not a cheap operation, so use this functionality
	 * wisely.
	 * 
	 ...
	 * width and height specified are larger than the framebuffer dimensions, the
	 * Texture will be padded accordingly. Pixels that fall outside of the current
	 * screen will have RGBA values of 0. Because of differences in screen and image
	 * origins the framebuffer contents should be flipped along the Y axis if you
	 * intend save them to disk as a bitmap. Flipping is not a cheap operation, so
	 * use this functionality wisely.
	 * 
	 * @param flipY whether to flip pixels along Y axis
	 */
