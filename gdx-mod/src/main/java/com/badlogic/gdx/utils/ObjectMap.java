...
/**
 * An unordered map. This implementation is a cuckoo hash map using 3 hashes,
 * random walking, and a small stash for problematic keys. Null keys are not
 * allowed. Null values are allowed. No allocation is done except when growing
 * the table size. <br>
 * <br>
...
public class ObjectMap<K, V> implements Iterable<ObjectMap.Entry<K, V>> {
	 ...
	 * 
	 * @param identity If true, uses == to compare the specified value with values
	 *                 in the map. If false, uses {@link #equals(Object)}.
	 */
	 ...
	 * the {@link Values} constructor for nested or multithreaded iteration.
	 */
	 ...
	 * the same iterator instance is returned each time this method is called. Use
	 * the {@link Keys} constructor for nested or multithreaded iteration.
	 */
	public Keys<K> keys() {
		if (keys1 == null) {
			keys1 = new Keys(this);
