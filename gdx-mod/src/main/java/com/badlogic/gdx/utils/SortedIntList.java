...
public class SortedIntList<E> implements Iterable<SortedIntList.Node<E>> {
	 ...
	 * @return Element replaced by newly inserted element, null if nothing was
	 *         replaced
	 */
	public E insert(int index, E value) {
		if (first != null) {
			Node<E> c = first;
			// iterate to the right until we can't move any further because the next number
	...
	class Iterator implements java.util.Iterator<Node<E>> {
		...
		public void remove() {
			...
			// returned yet assumably do nothing
