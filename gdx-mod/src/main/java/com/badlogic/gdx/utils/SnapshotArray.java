...
/**
 * Guarantees that array entries provided by {@link #begin()} between indexes 0
 * and {@link #size} at the time begin was called will not be modified until
 * {@link #end()} is called. If modification of the SnapshotArray occurs between
 * begin/end, the backing array is copied prior to the modification, ensuring
 * that the backing array that was returned by {@link #begin()} is unaffected.
 * To avoid allocation, an attempt is made to reuse any extra array created as a
 * result of this copy on subsequent copies.
 * <p>
...
public class SnapshotArray<T> extends Array<T> {
	...
	/**
	 * Releases the guarantee that the array returned by {@link #begin()} won't be
	 * modified.
	 */
	public void end() {
	...
	private void modified() {
		if (snapshot == null || snapshot != items)
			return;
		// Snapshot is in use, copy backing array to recycled array or create new
		// backing array.
