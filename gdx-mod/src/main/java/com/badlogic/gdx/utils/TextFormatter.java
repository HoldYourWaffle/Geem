...
class TextFormatter {
	 ...
	 * {@link MessageFormat} is used to process the pattern, meaning that the actual
	 * arguments are properly localized with the locale of this
	 * {@code TextFormatter}.
	 * <p>
	 ...
	 * pattern's placeholders are expected to be in the simplified form {0}, {1},
	 * {2} and so on and they will be replaced with the corresponding object from
	 * {@code args} converted to a string with {@code toString()}, so without taking
	 ...
	 * particular, it isn't always obvious to localizers whether single quotes need
	 * to be doubled or not. For this very reason we decided to offer the simpler
	 * escaping rule above without limiting the expressive power of message format
	 * patterns. So, if you're used to MessageFormat's syntax, remember that with
	 * {@code TextFormatter} single quotes never need to be escaped!
	 * 
	...
	// Also this code is optimized since a new string is returned only if something
	// has been replaced.
	 ...
	 * to a string with {@code toString()}, so without taking into account the
	 * locale.
	 * <p>
	 * This method only implements a small subset of the grammar supported by
	 * {@link java.text.MessageFormat}. Especially, placeholder are only made up of
	 * an index; neither the type nor the style are supported.
	 * <p>
