...
/**
 * A resizable, ordered array of objects with efficient add and remove at the
 * beginning and end. Values in the backing array may wrap back to the
 * beginning, making add and remove at the beginning and end O(1) (unless the
 * backing array needs to resize when adding). Deque functionality is provided
 * via {@link #removeLast()} and {@link #addFirst(Object)}.
 */
public class Queue<T> implements Iterable<T> {
	/**
	 * Contains the values in the queue. Head and tail indices go in a circle around
	 * this array, wrapping at the end.
	 */
	protected T[] values;

	/**
	 * Index of first element. Logically smaller than tail. Unless empty, it points
	 * to a valid element inside queue.
	 */
	protected int head = 0;

	/**
	 * Index of last element. Logically bigger than head. Usually points to an empty
	 * position, but points to the head when full (size == values.length).
	 */
	...
	/**
	 * Creates a new Queue which can hold 16 values without needing to resize
	 * backing array.
	 */
	public Queue() {
		this(16);
	...
	/**
	 * Creates a new Queue which can hold the specified number of values without
	 * needing to resize backing array. This creates backing array of the specified
	 * type via reflection, which is necessary only when accessing the backing array
	 * directly.
	 */
	...
	/**
	 * Append given object to the tail. (enqueue to tail) Unless backing array needs
	 * resizing, operates in O(1) time.
	 * 
	...
	/**
	 * Prepend given object to the head. (enqueue to head) Unless backing array
	 * needs resizing, operates in O(1) time.
	 * 
	 * @see #addLast(Object)
	 * @param object can be null
	 */
	...
	/**
	 * Returns the index of first occurrence of value in the queue, or -1 if no such
	 * value exists.
	 * 
	 * @param identity If true, == comparison will be used. If false, .equals()
	 *                 comparison will be used.
	 * @return An index of first occurrence of value in queue or -1 if no such value
	 *         exists
	 */
	...
	/**
	 * Retrieves the value in queue without removing it. Indexing is from the front
	 * to back, zero based. Therefore get(0) is the same as {@link #first()}.
	 * 
	...
	/**
	 * Removes all values from this queue. Values in backing array are set to null
	 * to prevent memory leak, so this operates in O(n).
	 */
	public void clear() {
		if (size == 0)
			return;
	...
	/**
	 * Returns an iterator for the items in the queue. Remove is supported. Note
	 * that the same iterator instance is returned each time this method is called.
	 * Use the {@link QueueIterator} constructor for nested or multithreaded
	 * iteration.
	 */
	@Override
