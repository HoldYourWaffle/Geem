 ...
 * specific task takes. Call {@link #start()} just before starting the task and
 * {@link #stop()} right after. You can do this multiple times if required.
 * Every render or update call {@link #tick()} to update the values. The
 * {@link #time} {@link FloatCounter} provides access to the minimum, maximum,
 * average, total and current time (in seconds) the task takes. Likewise for the
 * {@link #load} value, which is the percentage of the total time.
 * 
 * @author xoppa
 */
public class PerformanceCounter {
	 ...
	 * timing mechanism if needed, if you do so, you also need to update
	 * {@link #valid}.
	 */
	public float current = 0f;
	/**
	 * Flag to indicate that the current value is valid, you need to set this to
