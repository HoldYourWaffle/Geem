 ...
 * {@link AudioDevice} and {@link AudioRecorder} interfaces, create sound
 * effects via the {@link Sound} interface and play music streams via the
 * {@link Music} interface.
 * 
 * <p>
 ...
 * {@link ApplicationListener#pause()} method is called, and automatically
...
public interface Audio {
	/**
	 * Creates a new {@link AudioDevice} either in mono or stereo mode. The
	 * AudioDevice has to be disposed via its {@link AudioDevice#dispose()} method
	 ...
	 * gun shots or explosions. The Sound's audio data is retrieved from the file
	 * specified via the {@link FileHandle}. Note that the complete audio data is
	 * loaded into RAM. You should therefore not load big audio files with this
	 * methods. The current upper limit for decoded audio is 1 MB.
	 * </p>
	 * 
	 * <p>
	 ...
	 * Music instance has to be disposed if it is no longer used via the
	 * {@link Music#dispose()} method. Music instances are automatically paused when
