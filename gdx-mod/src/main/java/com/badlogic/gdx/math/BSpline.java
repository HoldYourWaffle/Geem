...
public class BSpline<T extends Vector<T>> implements Path<T> {
	...
	/**
	 * Calculates the cubic b-spline value for the given span (i) at the given
	 * position (u).
	 * 
	 * @param out        The Vector to set to the result.
	 * @param i          The span (0<=i<spanCount) spanCount = continuous ?
	 *                   points.length : points.length - 3 (cubic degree)
	...
	/**
	 * Calculates the cubic b-spline derivative for the given span (i) at the given
	 * position (u).
	 * 
	 * @param out        The Vector to set to the result.
	 * @param i          The span (0<=i<spanCount) spanCount = continuous ?
	...
	/**
	 * Calculates the n-degree b-spline value for the given span (i) at the given
	 * position (u).
	 * 
	 * @param out        The Vector to set to the result.
	 * @param i          The span (0<=i<spanCount) spanCount = continuous ?
	...
	public static <T extends Vector<T>> T calculate(final T out, final int i, final float u, final T[] points,
			final int degree, final boolean continuous, final T tmp) {
		switch (degree) {
		case 3:
			return cubic(out, i, u, points, continuous, tmp);
		}
	...
	/**
	 * Calculates the n-degree b-spline derivative for the given span (i) at the
	 * given position (u).
	 * 
	 * @param out        The Vector to set to the result.
	 * @param i          The span (0<=i<spanCount) spanCount = continuous ?
	...
	public BSpline set(final T[] controlPoints, final int degree, final boolean continuous) {
		...
		for (int i = 0; i < spanCount; i++)
			knots.add(calculate(controlPoints[0].cpy(), continuous ? i : (int) (i + 0.5f * degree), 0f, controlPoints,
	...
	public T derivativeAt(final T out, final int span, final float u) {
		return derivative(out, continuous ? span : (span + (int) (degree * 0.5f)), u, controlPoints, degree, continuous,
				tmp);
	}
