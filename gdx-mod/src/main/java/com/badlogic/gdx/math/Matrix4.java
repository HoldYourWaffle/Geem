...
public class Matrix4 implements Serializable {
	...
	/**
	 * XX: Typically the unrotated X component for scaling, also the cosine of the
	 * angle when rotated on the Y and/or Z axis. On Vector3 multiplication this
	 * value is multiplied with the source X component and added to the target X
	 * component.
	 */
	public static final int M00 = 0;
	/**
	 * XY: Typically the negative sine of the angle when rotated on the Z axis. On
	 * Vector3 multiplication this value is multiplied with the source Y component
	 * and added to the target X component.
	 */
	public static final int M01 = 4;
	/**
	 * XZ: Typically the sine of the angle when rotated on the Y axis. On Vector3
	 * multiplication this value is multiplied with the source Z component and added
	 ...
	 * multiplication this value is multiplied with the source X component and added
	 * to the target Y component.
	 */
	public static final int M10 = 1;
	/**
	 * YY: Typically the unrotated Y component for scaling, also the cosine of the
	 * angle when rotated on the X and/or Z axis. On Vector3 multiplication this
	 * value is multiplied with the source Y component and added to the target Y
	 * component.
	 */
	public static final int M11 = 5;
	/**
	 * YZ: Typically the negative sine of the angle when rotated on the X axis. On
	 * Vector3 multiplication this value is multiplied with the source Z component
	 * and added to the target Y component.
	 */
	public static final int M12 = 9;
	/**
	 * YW: Typically the translation of the Y component. On Vector3 multiplication
	 * this value is added to the target Y component.
	 */
	public static final int M13 = 13;
	/**
	 * ZX: Typically the negative sine of the angle when rotated on the Y axis. On
	 ...
	 * multiplication this value is multiplied with the source Y component and added
	 * to the target Z component.
	 */
	public static final int M21 = 6;
	/**
	 * ZZ: Typically the unrotated Z component for scaling, also the cosine of the
	 * angle when rotated on the X and/or Y axis. On Vector3 multiplication this
	 * value is multiplied with the source Z component and added to the target Z
	 * component.
	 */
	 ...
	 *               <a href="http://en.wikipedia.org/wiki/Row-major_order">column
	 ...
	 * 
	 * @param quaternion The quaternion to be copied. (The quaternion is not
	 *                   modified)
	 */
	 ...
	 * Sets the matrix to a rotation matrix representing the quaternion.
	 * 
	 * @param quaternionX The X component of the quaternion that is to be used to
	 *                    set this matrix.
	 * @param quaternionY The Y component of the quaternion that is to be used to
	 *                    set this matrix.
	 * @param quaternionZ The Z component of the quaternion that is to be used to
	 *                    set this matrix.
	 * @param quaternionW The W component of the quaternion that is to be used to
	 *                    set this matrix.
	 * @return This matrix for the purpose of chaining methods together.
	 */
	 ...
	 * quaternion.
	 * 
	 * @param translationX The X component of the translation that is to be used to
	 *                     set this matrix.
	 * @param translationY The Y component of the translation that is to be used to
	 *                     set this matrix.
	 * @param translationZ The Z component of the translation that is to be used to
	 *                     set this matrix.
	 * @param quaternionX  The X component of the quaternion that is to be used to
	 *                     set this matrix.
	 * @param quaternionY  The Y component of the quaternion that is to be used to
	 ...
	 * @param quaternionW  The W component of the quaternion that is to be used to
	 *                     set this matrix.
	 * @param scaleX       The X component of the scaling that is to be used to set
	 *                     this matrix.
	 * @param scaleY       The Y component of the scaling that is to be used to set
	 *                     this matrix.
	 * @param scaleZ       The Z component of the scaling that is to be used to set
	 *                     this matrix.
	 * @return This matrix for the purpose of chaining methods together.
	 */
	public Matrix4 set(float translationX, float translationY, float translationZ, float quaternionX, float quaternionY,
	 ...
	 * representing the translation of any point that is multiplied by this matrix.
	 * 
	 ...
	 * the angle in degrees for the height, the field of view for the width will be
	 ...
	 * to the lower left and upper right corners of the viewport. This allows to
	 * create projection matrix with off-center vanishing point.
	 * 
	 * @param left
	 * @param right
	 * @param bottom
	 ...
	 * and the scaling vector in the diagonal.
	 * 
	 ...
	 * @param w     Weight of this transform; weight of the other transform is (1 -
	 *              w)
	 * @return This matrix for chaining
	 */
	 ...
	 * Does not destroy the data contained in t or w; Sum of w_i must be equal to 1,
	 ...
	 * @param normalizeAxes True to normalize the axes, necessary when the matrix
	 *                      might also include scaling.
	 ...
	 * 
	 * #define M00 0 #define M01 4 #define M02 8 #define M03 12 #define M10 1
	 * #define M11 5 #define M12 9 #define M13 13 #define M20 2 #define M21 6
	 * #define M22 10 #define M23 14 #define M30 3 #define M31 7 #define M32 11
	 * #define M33 15
	 * 
	 * static inline void matrix4_mul(float* mata, float* matb) { float tmp[16];
	 * tmp[M00] = mata[M00] * matb[M00] + mata[M01] * matb[M10] + mata[M02] *
	 * matb[M20] + mata[M03] * matb[M30]; tmp[M01] = mata[M00] * matb[M01] +
	 * mata[M01] * matb[M11] + mata[M02] * matb[M21] + mata[M03] * matb[M31];
	 * tmp[M02] = mata[M00] * matb[M02] + mata[M01] * matb[M12] + mata[M02] *
	 * matb[M22] + mata[M03] * matb[M32]; tmp[M03] = mata[M00] * matb[M03] +
	 * mata[M01] * matb[M13] + mata[M02] * matb[M23] + mata[M03] * matb[M33];
	 * tmp[M10] = mata[M10] * matb[M00] + mata[M11] * matb[M10] + mata[M12] *
	 * matb[M20] + mata[M13] * matb[M30]; tmp[M11] = mata[M10] * matb[M01] +
	 * mata[M11] * matb[M11] + mata[M12] * matb[M21] + mata[M13] * matb[M31];
	 * tmp[M12] = mata[M10] * matb[M02] + mata[M11] * matb[M12] + mata[M12] *
	 * matb[M22] + mata[M13] * matb[M32]; tmp[M13] = mata[M10] * matb[M03] +
	 * mata[M11] * matb[M13] + mata[M12] * matb[M23] + mata[M13] * matb[M33];
	 * tmp[M20] = mata[M20] * matb[M00] + mata[M21] * matb[M10] + mata[M22] *
	 * matb[M20] + mata[M23] * matb[M30]; tmp[M21] = mata[M20] * matb[M01] +
	 * mata[M21] * matb[M11] + mata[M22] * matb[M21] + mata[M23] * matb[M31];
	 * tmp[M22] = mata[M20] * matb[M02] + mata[M21] * matb[M12] + mata[M22] *
	 * matb[M22] + mata[M23] * matb[M32]; tmp[M23] = mata[M20] * matb[M03] +
	 * mata[M21] * matb[M13] + mata[M22] * matb[M23] + mata[M23] * matb[M33];
	 * tmp[M30] = mata[M30] * matb[M00] + mata[M31] * matb[M10] + mata[M32] *
	 * matb[M20] + mata[M33] * matb[M30]; tmp[M31] = mata[M30] * matb[M01] +
	 * mata[M31] * matb[M11] + mata[M32] * matb[M21] + mata[M33] * matb[M31];
	 * tmp[M32] = mata[M30] * matb[M02] + mata[M31] * matb[M12] + mata[M32] *
	 * matb[M22] + mata[M33] * matb[M32]; tmp[M33] = mata[M30] * matb[M03] +
	 * mata[M31] * matb[M13] + mata[M32] * matb[M23] + mata[M33] * matb[M33];
	 * memcpy(mata, tmp, sizeof(float) * 16); }
	 * 
	 * static inline float matrix4_det(float* val) { return val[M30] * val[M21] *
	 * val[M12] * val[M03] - val[M20] * val[M31] * val[M12] * val[M03] - val[M30] *
	 * val[M11] val[M22] * val[M03] + val[M10] * val[M31] * val[M22] * val[M03] +
	 * val[M20] * val[M11] * val[M32] * val[M03] - val[M10] val[M21] * val[M32] *
	 * val[M03] - val[M30] * val[M21] * val[M02] * val[M13] + val[M20] * val[M31] *
	 * val[M02] * val[M13] + val[M30] * val[M01] * val[M22] * val[M13] - val[M00] *
	 * val[M31] * val[M22] * val[M13] - val[M20] * val[M01] * val[M32] val[M13] +
	 * val[M00] * val[M21] * val[M32] * val[M13] + val[M30] * val[M11] * val[M02] *
	 * val[M23] - val[M10] * val[M31] val[M02] * val[M23] - val[M30] * val[M01] *
	 * val[M12] * val[M23] + val[M00] * val[M31] * val[M12] * val[M23] + val[M10]
	 * val[M01] * val[M32] * val[M23] - val[M00] * val[M11] * val[M32] * val[M23] -
	 * val[M20] * val[M11] * val[M02] * val[M33] + val[M10] * val[M21] * val[M02] *
	 * val[M33] + val[M20] * val[M01] * val[M12] * val[M33] - val[M00] * val[M21] *
	 * val[M12] val[M33] - val[M10] * val[M01] * val[M22] * val[M33] + val[M00] *
	 * val[M11] * val[M22] * val[M33]; }
	 * 
	 * static inline bool matrix4_inv(float* val) { float tmp[16]; float l_det =
	 * matrix4_det(val); if (l_det == 0) return false; tmp[M00] = val[M12] *
	 * val[M23] * val[M31] - val[M13] * val[M22] * val[M31] + val[M13] * val[M21] *
	 * val[M32] - val[M11] val[M23] * val[M32] - val[M12] * val[M21] * val[M33] +
	 * val[M11] * val[M22] * val[M33]; tmp[M01] = val[M03] * val[M22] * val[M31] -
	 * val[M02] * val[M23] * val[M31] - val[M03] * val[M21] * val[M32] + val[M01]
	 * val[M23] * val[M32] + val[M02] * val[M21] * val[M33] - val[M01] * val[M22] *
	 * val[M33]; tmp[M02] = val[M02] * val[M13] * val[M31] - val[M03] * val[M12] *
	 * val[M31] + val[M03] * val[M11] * val[M32] - val[M01] val[M13] * val[M32] -
	 * val[M02] * val[M11] * val[M33] + val[M01] * val[M12] * val[M33]; tmp[M03] =
	 * val[M03] * val[M12] * val[M21] - val[M02] * val[M13] * val[M21] - val[M03] *
	 * val[M11] * val[M22] + val[M01] val[M13] * val[M22] + val[M02] * val[M11] *
	 * val[M23] - val[M01] * val[M12] * val[M23]; tmp[M10] = val[M13] * val[M22] *
	 * val[M30] - val[M12] * val[M23] * val[M30] - val[M13] * val[M20] * val[M32] +
	 * val[M10] val[M23] * val[M32] + val[M12] * val[M20] * val[M33] - val[M10] *
	 * val[M22] * val[M33]; tmp[M11] = val[M02] * val[M23] * val[M30] - val[M03] *
	 * val[M22] * val[M30] + val[M03] * val[M20] * val[M32] - val[M00] val[M23] *
	 * val[M32] - val[M02] * val[M20] * val[M33] + val[M00] * val[M22] * val[M33];
	 * tmp[M12] = val[M03] * val[M12] * val[M30] - val[M02] * val[M13] * val[M30] -
	 * val[M03] * val[M10] * val[M32] + val[M00] val[M13] * val[M32] + val[M02] *
	 * val[M10] * val[M33] - val[M00] * val[M12] * val[M33]; tmp[M13] = val[M02] *
	 * val[M13] * val[M20] - val[M03] * val[M12] * val[M20] + val[M03] * val[M10] *
	 * val[M22] - val[M00] val[M13] * val[M22] - val[M02] * val[M10] * val[M23] +
	 * val[M00] * val[M12] * val[M23]; tmp[M20] = val[M11] * val[M23] * val[M30] -
	 * val[M13] * val[M21] * val[M30] + val[M13] * val[M20] * val[M31] - val[M10]
	 * val[M23] * val[M31] - val[M11] * val[M20] * val[M33] + val[M10] * val[M21] *
	 * val[M33]; tmp[M21] = val[M03] * val[M21] * val[M30] - val[M01] * val[M23] *
	 * val[M30] - val[M03] * val[M20] * val[M31] + val[M00] val[M23] * val[M31] +
	 * val[M01] * val[M20] * val[M33] - val[M00] * val[M21] * val[M33]; tmp[M22] =
	 * val[M01] * val[M13] * val[M30] - val[M03] * val[M11] * val[M30] + val[M03] *
	 * val[M10] * val[M31] - val[M00] val[M13] * val[M31] - val[M01] * val[M10] *
	 * val[M33] + val[M00] * val[M11] * val[M33]; tmp[M23] = val[M03] * val[M11] *
	 * val[M20] - val[M01] * val[M13] * val[M20] - val[M03] * val[M10] * val[M21] +
	 * val[M00] val[M13] * val[M21] + val[M01] * val[M10] * val[M23] - val[M00] *
	 * val[M11] * val[M23]; tmp[M30] = val[M12] * val[M21] * val[M30] - val[M11] *
	 * val[M22] * val[M30] - val[M12] * val[M20] * val[M31] + val[M10] val[M22] *
	 * val[M31] + val[M11] * val[M20] * val[M32] - val[M10] * val[M21] * val[M32];
	 * tmp[M31] = val[M01] * val[M22] * val[M30] - val[M02] * val[M21] * val[M30] +
	 * val[M02] * val[M20] * val[M31] - val[M00] val[M22] * val[M31] - val[M01] *
	 * val[M20] * val[M32] + val[M00] * val[M21] * val[M32]; tmp[M32] = val[M02] *
	 * val[M11] * val[M30] - val[M01] * val[M12] * val[M30] - val[M02] * val[M10] *
	 * val[M31] + val[M00] val[M12] * val[M31] + val[M01] * val[M10] * val[M32] -
	 * val[M00] * val[M11] * val[M32]; tmp[M33] = val[M01] * val[M12] * val[M20] -
	 * val[M02] * val[M11] * val[M20] + val[M02] * val[M10] * val[M21] - val[M00]
	 * val[M12] * val[M21] - val[M01] * val[M10] * val[M22] + val[M00] * val[M11] *
	 * val[M22];
	 * 
	 * float inv_det = 1.0f / l_det; val[M00] = tmp[M00] * inv_det; val[M01] =
	 * tmp[M01] * inv_det; val[M02] = tmp[M02] * inv_det; val[M03] = tmp[M03] *
	 * inv_det; val[M10] = tmp[M10] * inv_det; val[M11] = tmp[M11] * inv_det;
	 * val[M12] = tmp[M12] * inv_det; val[M13] = tmp[M13] * inv_det; val[M20] =
	 * tmp[M20] * inv_det; val[M21] = tmp[M21] * inv_det; val[M22] = tmp[M22] *
	 * inv_det; val[M23] = tmp[M23] * inv_det; val[M30] = tmp[M30] * inv_det;
	 * val[M31] = tmp[M31] * inv_det; val[M32] = tmp[M32] * inv_det; val[M33] =
	 * tmp[M33] * inv_det; return true; }
	 * 
	 * static inline void matrix4_mulVec(float* mat, float* vec) { float x = vec[0]
	 * * mat[M00] + vec[1] * mat[M01] + vec[2] * mat[M02] + mat[M03]; float y =
	 * vec[0] * mat[M10] + vec[1] * mat[M11] + vec[2] * mat[M12] + mat[M13]; float z
	 * = vec[0] * mat[M20] + vec[1] * mat[M21] + vec[2] * mat[M22] + mat[M23];
	 * vec[0] = x; vec[1] = y; vec[2] = z; }
	 * 
	 * static inline void matrix4_proj(float* mat, float* vec) { float inv_w = 1.0f
	 * / (vec[0] * mat[M30] + vec[1] * mat[M31] + vec[2] * mat[M32] + mat[M33]);
	 * float x = (vec[0] * mat[M00] + vec[1] * mat[M01] + vec[2] * mat[M02] +
	 * mat[M03]) * inv_w; float y = (vec[0] * mat[M10] + vec[1] * mat[M11] + vec[2]
	 * * mat[M12] + mat[M13]) * inv_w; float z = (vec[0] * mat[M20] + vec[1] *
	 * mat[M21] + vec[2] * mat[M22] + mat[M23]) * inv_w; vec[0] = x; vec[1] = y;
	 * vec[2] = z; }
	 * 
	 * static inline void matrix4_rot(float* mat, float* vec) { float x = vec[0] *
	 * mat[M00] + vec[1] * mat[M01] + vec[2] * mat[M02]; float y = vec[0] * mat[M10]
	 ...
	 * {@link Matrix4#val}. This is the same as {@link Matrix4#mul(Matrix4)}.
	 *
	 ...
	 * vector array is assumed to hold a 3-component vector, with x being the first
	 * element, y being the second and z being the last component. The result is
	 * stored in the vector array. This is the same as {@link Vector3#mul(Matrix4)}.
	 * 
	 * @param mat the matrix
	 ...
	 * hold a 4x4 column major matrix as you can get from {@link Matrix4#val}. The
	 * vectors array is assumed to hold 3-component vectors. Offset specifies the
	 * offset into the array where the x-component of the first vector is located.
	 ...
	 * vectors and must be >= 3. This is the same as {@link Vector3#mul(Matrix4)}
	 ...
	 */
	public static native void mulVec(float[] mat, float[] vecs, int offset, int numVecs,
			int stride) /*-{ }-*/; /*
	 ...
	 * {@link Matrix4#val}. The vector array is assumed to hold a 3-component
	 * vector, with x being the first element, y being the second and z being the
	 * last component. The result is stored in the vector array. This is the same as
	 ...
	 * from {@link Matrix4#val}. The vectors array is assumed to hold 3-component
	 * vectors. Offset specifies the offset into the array where the x-component of
	 * the first vector is located. The numVecs parameter specifies the number of
	 * vectors stored in the vectors array. The stride parameter specifies the
	 * number of floats between subsequent vectors and must be >= 3. This is the
	 * same as {@link Vector3#prj(Matrix4)} applied to multiple vectors.
	 * 
	 * @param mat     the matrix
	 * @param vecs    the vectors
	 * @param offset  the offset into the vectors array
	 ...
	 * @param stride  the stride between vectors in floats
	 */
	public static native void prj(float[] mat, float[] vecs, int offset, int numVecs,
			int stride) /*-{ }-*/; /*
									 * float* vecPtr = vecs + offset; for(int i = 0; i < numVecs; i++) {
	 ...
	 * The matrix array is assumed to hold a 4x4 column major matrix as you can get
	 * from {@link Matrix4#val}. The vector array is assumed to hold a 3-component
	 * vector, with x being the first element, y being the second and z being the
	 * last component. The result is stored in the vector array. This is the same as
	 ...
	 * vectors stored in the vectors array. The stride parameter specifies the
	 * number of floats between subsequent vectors and must be >= 3. This is the
	 * same as {@link Vector3#rot(Matrix4)} applied to multiple vectors.
	 * 
	 * @param mat     the matrix
	 * @param vecs    the vectors
	 * @param offset  the offset into the vectors array
	 ...
	 * @param stride  the stride between vectors in floats
	 */
	public static native void rot(float[] mat, float[] vecs, int offset, int numVecs,
			int stride) /*-{ }-*/; /*
									 * float* vecPtr = vecs + offset; for(int i = 0; i < numVecs; i++) {
