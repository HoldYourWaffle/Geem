...
public class Frustum {
	...
	public boolean boundsInFrustum(BoundingBox bounds) {
		for (Plane plane : planes) {
			if (plane.testPoint(bounds.getCorner000(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner001(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner010(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner011(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner100(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner101(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner110(tmpV)) != PlaneSide.Back)
				continue;
			if (plane.testPoint(bounds.getCorner111(tmpV)) != PlaneSide.Back)
				continue;
			return false;
		}
