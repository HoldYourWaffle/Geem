 ...
 * or Android assets directory. FileHandles are created via a {@link Files}
 * instance.
 * 
 * Because some of the file types are backed by composite files and may be
 * compressed (for example, if they are in an Android .apk or are found via the
 * classpath), the methods for extracting a {@link #path()} or {@link #file()}
 * may not be appropriate for all types. Use the Reader or Stream methods here
 * to hide these dependencies from your platform independent code.
 * 
 * @author mzechner
 * @author Nathan Sweet
 */
public class FileHandle {
	 ...
	 * in case you write something cross-platform. Use the {@link Files} interface
	 * instead.
	 * 
	 ...
	 * {@link FileType#External} file handles.
	 */
	public File file() {
	 ...
	 * 
	 * @throws GdxRuntimeException if the file handle represents a directory,
	 ...
	 * if necessary.
	 * 
	 * @param append If false, this file will be overwritten if it exists, otherwise
	 *               it will be appended.
	 * @throws GdxRuntimeException if this file handle represents a directory, if it
	 *                             is a {@link FileType#Classpath} or
	 *                             {@link FileType#Internal} file, or if it could
	 *                             not be written.
	 */
	 ...
	 * {@link FileType#Internal} handle to a directory on the classpath will return
	 * a zero length array.
	 * 
	 * @throws GdxRuntimeException if this file is an {@link FileType#Classpath}
	 *                             file.
	 */
	 ...
	 * and not a directory. On the desktop, an {@link FileType#Internal} handle to a
	 ...
	 * directory. On the desktop, an {@link FileType#Internal} handle to a directory
	 * on the classpath will return a zero length array.
	 * 
	 * @throws GdxRuntimeException if this file is an {@link FileType#Classpath}
	 *                             file.
	 */
	 ...
	 * will return false. On the desktop, an {@link FileType#Internal} handle to a
	 ...
	 * 2) if the destination is a directory, this file is copied into it, or 3) if
	 * the destination doesn't exist, {@link #mkdirs()} is called on the
	 * destination's parent and this file is copied into it with a new name. If this
	 * handle is a directory, then 1) if the destination is a file,
	 * GdxRuntimeException is thrown, or 2) if the destination is a directory, this
	 * directory is copied into it recursively, overwriting existing files, or 3) if
	 * the destination doesn't exist, {@link #mkdirs()} is called on the destination
	 * and this directory is copied into it recursively.
	 * 
	 * @throws GdxRuntimeException if the destination file handle is a
	 *                             {@link FileType#Classpath} or
	 *                             {@link FileType#Internal} file, or copying
	 *                             failed.
	 */
	 ...
	 * exists.
	 * 
	 * @throws GdxRuntimeException if the source or destination file handle is a
	 *                             {@link FileType#Classpath} or
	 *                             {@link FileType#Internal} file.
	 */
	public void moveTo(FileHandle dest) {
		...
		case Absolute:
		case External:
			// Try rename for efficiency and to change case on case-insensitive file
			// systems.
