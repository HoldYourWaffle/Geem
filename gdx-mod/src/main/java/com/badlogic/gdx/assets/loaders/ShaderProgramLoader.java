...
/**
 * {@link AssetLoader} for {@link ShaderProgram} instances loaded from text
 * files. If the file suffix is ".vert", it is assumed to be a vertex shader,
 * and a fragment shader is found using the same file name with a ".frag"
 * suffix. And vice versa if the file suffix is ".frag". These default suffixes
 * can be changed in the ShaderProgramLoader constructor.
 * <p>
 * For all other file suffixes, the same file is used for both (and therefore
 * should internally distinguish between the programs using preprocessor
 * directives and {@link ShaderProgram#prependVertexCode} and
 * {@link ShaderProgram#prependFragmentCode}).
 * <p>
 * The above default behavior for finding the files can be overridden by
 * explicitly setting the file names in a {@link ShaderProgramParameter}. The
...
public class ShaderProgramLoader
	...
	static public class ShaderProgramParameter extends AssetLoaderParameters<ShaderProgram> {
		...
		/**
		 * Code that is always added to the vertex shader code. This is added as-is, and
		 * you should include a newline (`\n`) if needed.
		 * {@linkplain ShaderProgram#prependVertexCode} is placed before this code.
		 */
		 ...
		 * {@linkplain ShaderProgram#prependFragmentCode} is placed before this code.
		 */
