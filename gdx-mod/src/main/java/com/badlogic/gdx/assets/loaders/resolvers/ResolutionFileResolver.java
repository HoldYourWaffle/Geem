...
/**
 * This {@link FileHandleResolver} uses a given list of {@link Resolution}s to
 * determine the best match based on the current Screen size. An example of how
 * this resolver works:
 * 
 * <p>
 * Let's assume that we have only a single {@link Resolution} added to this
 ...
 * 
 * <p>
 * One would now supply a file to be found to the resolver. For this example, we
 * assume it is "{@code textures/walls/brick.png}". Since there is only a single
 * {@link Resolution}, this will be the best match for any screen size. The
 * resolver will now try to find the file in the following ways:
 * </p>
 * 
 * <ul>
 ...
 * 
 * <p>
 * The files are ultimately resolved via the given {{@link #baseResolver}. In
 * case the first version cannot be resolved, the fallback will try to search
 * for the file without the resolution folder.
 * </p>
 */
public class ResolutionFileResolver implements FileHandleResolver {
	...
	public static class Resolution {
		 ...
		 * @param folder         The name of the folder, where the assets which fit this
		 *                       resolution, are located.
		 */
	 ...
	 * 
	 * @param baseResolver The {@link FileHandleResolver} that will ultimately used
	 *                     to resolve the file.
	 * @param descriptors  A list of {@link Resolution}s. At least one has to be
	 *                     supplied.
	 */
