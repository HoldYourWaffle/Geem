...
public abstract class BaseTmxMapLoader<P extends AssetLoaderParameters<TiledMap>>
	...
	public static class Parameters extends AssetLoaderParameters<TiledMap> {
		...
		/**
		 * Whether to convert the objects' pixel position and size to the equivalent in
		 * tile space.
		 **/
		public boolean convertObjectToTileSpace = false;
		/**
		 * Whether to flip all Y coordinates so that Y positive is down. All LibGDX
		 * renderers require flipped Y coordinates, and thus flipY set to true. This
		 * parameter is included for non-rendering related purposes of TMX files, or
		 * custom renderers.
		 */
		...
		if (encoding.equals("csv")) {
			...
			if (true)
				if (encoding.equals("base64")) {
					...
					// any other value of 'encoding' is one we're not aware of, probably a feature
					// of a future version of Tiled
					// or another editor
