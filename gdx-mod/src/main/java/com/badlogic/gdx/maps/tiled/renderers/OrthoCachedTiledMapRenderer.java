 ...
 * controlled by {@link #setOverCache(float)}. When the view reaches the edge of
 * the cached tiles, the cache is rebuilt at the new view position.
 * <p>
 * This class may have poor performance when tiles are often changed
 * dynamically, since the cache must be rebuilt after each change.
 * 
...
public class OrthoCachedTiledMapRenderer implements TiledMapRenderer, Disposable {
	...
	/**
	 * Sets the percentage of the view that is cached in each direction. Default is
	 * 0.5.
	 * <p>
	 * Eg, 0.75 will cache 75% of the width of the view to the left and right of the
	 * view, and 75% of the height of the view above and below the view.
	 */
	 ...
	 * smaller are never culled from the visible portion of the view. Default is
	 * 0,0.
	 * <p>
	 * The amount of tiles cached is computed using
	 * <code>(view size + max tile size) * overCache</code>, meaning the max tile
	 * size increases the amount cached and possibly {@link #setOverCache(float)}
	 * can be reduced.
	 * <p>
	 * If the view size and {@link #setOverCache(float)} are configured so the size
	 * of the cached tiles is always larger than the largest tile size, this setting
	 * is not needed.
	 */
