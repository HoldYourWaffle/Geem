...
public class HexagonalTiledMapRenderer extends BatchTiledMapRenderer {
	...
	/**
	 * the parameter defining the shape of the hexagon from tiled. more specifically
	 * it represents the length of the sides that are parallel to the stagger axis.
	 * e.g. with respect to the stagger axis a value of 0 results in a rhombus
	 * shape, while a value equal to the tile length/height represents a square
	 * shape and a value of 0.5 represents a regular hexagon if tile length equals
	 * tile height
	 */
	...
	public void renderTileLayer(TiledMapTileLayer layer) {
		...
		if (staggerAxisX) {
			...
			final int col1 = Math.max(0,
					(int) (((viewBounds.x - tileWidthLowerCorner - layerOffsetY) / tileWidthUpperCorner)));
			final int col2 = Math.min(layerWidth,
					(int) ((viewBounds.x + viewBounds.width + tileWidthUpperCorner - layerOffsetY)
							/ tileWidthUpperCorner));

			// depending on the stagger index either draw all even before the odd or vice
			// versa
			...
			final int row1 = Math.max(0,
					(int) (((viewBounds.y - tileHeightLowerCorner - layerOffsetX) / tileHeightUpperCorner)));
			final int row2 = Math.min(layerHeight,
