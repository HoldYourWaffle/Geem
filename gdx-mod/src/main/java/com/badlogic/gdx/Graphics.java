 ...
 * configuration, access to {@link GL20} and {@link GL30} are provided here.
 * <p>
 ...
 * {@link Batch} and so on. All these classes are managed, meaning they don't
 * need to be reloaded on a context loss. Explore the com.badlogic.gdx.graphics
 * package for more classes that might come in handy.
 * 
 * @author mzechner
 */
public interface Graphics {
	...
	public static class BufferFormat {
		...
		/**
		 * whether coverage sampling anti-aliasing is used. in that case you have to
		 * clear the coverage buffer as well!
		 */
	...
	/**
	 * Returns whether OpenGL ES 3.0 is available. If it is you can get an instance
	 * of {@link GL30} via {@link #getGL30()} to access OpenGL ES 3.0 functionality.
	 * Note that this functionality will only be available if you instructed the
	...
	/**
	 * Returns the id of the current frame. The general contract of this method is
	 * that the id is incremented only when the application is in the running state
	 * right before calling the {@link ApplicationListener#render()} method. Also,
	 * the id of the first frame is 0; the id of subsequent frames is guaranteed to
	 * take increasing values for 2<sup>63</sup>-1 rendering cycles.
	 * 
	 ...
	 * the same conventions as android.util.DisplayMetrics#density, where one DIP is
	 * one pixel on an approximately 160 dpi screen. Thus on a 160dpi screen this
	 ...
	 * {@link Graphics#setFullscreenMode(DisplayMode)}
	 *
	...
	/**
	 * @return the supported fullscreen {@link DisplayMode}(s) of the monitor the
	 *         window is on
	 */
	public DisplayMode[] getDisplayModes();
	
	/**
	 * @return the supported fullscreen {@link DisplayMode}s of the given
	 *         {@link Monitor}
	 */
	...
	/**
	 * Sets the window decoration as enabled or disabled. On Android, this will
	 * enable/disable the menu bar.
	 *
	 * Note that immediate behavior of this method may vary depending on the
	 * implementation. It may be necessary for the window to be recreated in order
	 * for the changes to take effect. Consult the documentation for the backend in
	 * use for more information.
	 *
	 * Supported on all GDX desktop backends and on Android (to disable the menu
	 * bar).
	 *
	 * @param undecorated true if the window border or status bar should be hidden.
	 *                    false otherwise.
	 */
	...
	/**
	 * Enable/Disable vsynching. This is a best-effort attempt which might not work
	 * on all platforms.
	 *
	 ...
	 * non-continuously, the following events will trigger a redraw:
	 *
	 * <ul>
	 ...
	 * {@link Application#postRunnable(Runnable)}. In the case of a multi-window
	 * app, all windows will request rendering if a runnable is posted to the
	 * application. To avoid this, post a runnable to the window instead.</li>
	 * </ul>
	 *
	 * Life-cycle events will also be reported as usual, see
	 * {@link ApplicationListener}. This method can be called from any thread.
	 *
	 ...
	 * {@link com.badlogic.gdx.graphics.Pixmap}. The Pixmap must be in RGBA8888
	 * format, width & height must be powers-of-two greater than zero (not
	 * necessarily equal) and of a certain minimum size (32x32 is a safe bet), and
	 * alpha transparency must be single-bit (i.e., 0x00 or 0xFF only). This
	 * function returns a Cursor object that can be set as the system cursor by
	 * calling {@link #setCursor(Cursor)} .
	 *
	 * @param pixmap   the mouse cursor image as a
	 *                 {@link com.badlogic.gdx.graphics.Pixmap}
	 * @param xHotspot the x location of the hotspot pixel within the cursor image
	 *                 (origin top-left corner)
	 * @param yHotspot the y location of the hotspot pixel within the cursor image
	 *                 (origin top-left corner)
	 * @return a cursor object that can be used by calling
	 *         {@link #setCursor(Cursor)} or null if not supported
	 */
	 ...
	 * mouse cursor image to the image represented by the
	 * {@link com.badlogic.gdx.graphics.Cursor}. It is recommended to call this
	 * function in the main render thread, and maximum one time per frame.
	 *
