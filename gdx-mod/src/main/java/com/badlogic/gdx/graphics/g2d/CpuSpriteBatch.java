...
/**
 * CpuSpriteBatch behaves like SpriteBatch, except it doesn't flush
 * automatically whenever the transformation matrix changes. Instead, the
 * vertices get adjusted on subsequent draws to match the running batch. This
 * can improve performance through longer batches, for example when drawing
 * Groups with transform enabled.
 *
 * @see SpriteBatch#renderCalls
 * @see com.badlogic.gdx.scenes.scene2d.Group#setTransform(boolean)
 *      Group.setTransform()
...
public class CpuSpriteBatch extends SpriteBatch {
	...
	/**
	 * <p>
	 * Flushes the batch and realigns the real matrix on the GPU. Subsequent draws
	 * won't need adjustment and will be slightly faster as long as the transform
	 * matrix is not {@link #setTransformMatrix(Matrix4) changed}.
	 * </p>
	 * <p>
	 ...
	 * inside a {@link #begin()}/{@link #end()} block, the current batch is
	 * <em>not</em> flushed to the GPU. Instead, for every subsequent draw() the
	 * vertices will be transformed on the CPU to match the original batch matrix.
	 * This adjustment must be performed until the matrices are realigned by
	 * restoring the original matrix, or by calling
	 * {@link #flushAndSyncTransformMatrix()}.
	 */
	@Override
	 ...
	 * This adjustment must be performed until the matrices are realigned by
	 * restoring the original matrix, or by calling
	 * {@link #flushAndSyncTransformMatrix()} or {@link #end()}.
	 */
	...
	public void draw(Texture texture, float x, float y, float originX, float originY, float width, float height,
		...
		if (!adjustNeeded) {
			...
			drawAdjusted(texture, x, y, originX, originY, width, height, scaleX, scaleY, rotation, srcX, srcY, srcWidth,
					srcHeight, flipX, flipY);
		}
	}
	...
	public void draw(Texture texture, float x, float y, int srcX, int srcY, int srcWidth, int srcHeight) {
		if (!adjustNeeded) {
			...
			drawAdjusted(texture, x, y, 0, 0, srcWidth, srcHeight, 1, 1, 0, srcX, srcY, srcWidth, srcHeight, false,
					false);
		}
	}
	...
	public void draw(Texture texture, float x, float y) {
		if (!adjustNeeded) {
			...
			drawAdjusted(texture, x, y, 0, 0, texture.getWidth(), texture.getHeight(), 1, 1, 0, 0, 1, 1, 0, false,
					false);
		}
	}
	...
	private void drawAdjusted(TextureRegion region, float x, float y, float originX, float originY, float width,
			float height, float scaleX, float scaleY, float rotation) {
		// v must be flipped
		drawAdjustedUV(region.texture, x, y, originX, originY, width, height, scaleX, scaleY, rotation, region.u,
				region.v2, region.u2, region.v, false, false);
	}
	
	private void drawAdjusted(Texture texture, float x, float y, float originX, float originY, float width,
		...
		drawAdjustedUV(texture, x, y, originX, originY, width, height, scaleX, scaleY, rotation, u, v, u2, v2, flipX,
				flipY);
	}
	
	private void drawAdjustedUV(Texture texture, float x, float y, float originX, float originY, float width,
			float height, float scaleX, float scaleY, float rotation, float u, float v, float u2, float v2,
			boolean flipX, boolean flipY) {
		if (!drawing)
