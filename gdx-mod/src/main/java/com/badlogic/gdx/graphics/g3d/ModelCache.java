...
/**
 * ModelCache tries to combine multiple render calls into a single render call
 * by merging them where possible. Can be used for multiple type of models (e.g.
 * varying vertex attributes or materials), the ModelCache will combine where
 * possible. Can be used dynamically (e.g. every frame) or statically (e.g. to
 * combine part of scenery). Be aware that any combined vertices are directly
 * transformed, therefore the resulting {@link Renderable#worldTransform} might
 * not be suitable for sorting anymore (such as the default sorter of ModelBatch
 * does).
 * 
 * @author Xoppa
 */
public class ModelCache implements Disposable, RenderableProvider {
	 ...
	 * Depending on the implementation it might add memory optimizations as well.
	 * Call the {@link #obtain(VertexAttributes, int, int)} method to obtain a mesh
	 * which can at minimum the specified amount of vertices and indices. Call the
	 * {@link #flush()} method to flush the pool ant release all previously obtained
	 * meshes.
	 */
	public interface MeshPool extends Disposable {
		/**
		 * Will try to reuse or, when not possible to reuse, optionally create a
		 * {@link Mesh} that meets the specified criteria.
		 * 
		 * @param vertexAttributes the vertex attributes of the mesh to obtain
		 * @param vertexCount      the minimum amount vertices the mesh should be able
		 *                         to store
		 * @param indexCount       the minimum amount of indices the mesh should be able
		 *                         to store
	 ...
	 * (32k) size. Use this when for dynamic caching where you need to obtain meshes
	 * very frequently (typically every frame).
	 * 
	 * @author Xoppa
	 */
	public static class SimpleMeshPool implements MeshPool {
		...
		public Mesh obtain(VertexAttributes vertexAttributes, int vertexCount, int indexCount) {
			...
			vertexCount = 1 + Short.MAX_VALUE;
			indexCount = Math.max(1 + Short.MAX_VALUE, 1 << (32 - Integer.numberOfLeadingZeros(indexCount - 1)));
	 ...
	 * implementation for you use-case, but should be good to start with.
	 */
	public ModelCache() {
	 ...
	 * will be called with the camera specified in {@link #begin(Camera)}. By
	 * default this will be null. The sorter is important for optimizing the cache.
	 * For the best result, make sure that renderables that can be merged are next
	 * to each other.
	 */
	 ...
	 * made. Calling this method will clear the cache and prepare it for creating a
	 * new cache. The cache is not valid until the call to {@link #end()} is made.
	 * Use one of the add methods (e.g. {@link #add(Renderable)} or
	 ...
	 * {@link #add(RenderableProvider)}) to add renderables to the cache.
	 * 
	 * @param camera The {@link Camera} that will passed to the
	 *               {@link RenderableSorter}
	 */
	 ...
	 * {@link #begin()}). Calling this method will process all renderables added
	 * using one of the add(...) methods and will combine them if possible.
	 */
	public void end() {
		if (!building)
	 ...
	 * (depending on possibilities) be used by reference and should not change while
	 * the cache is used. If the {@link Renderable#bones} member is not null then
	 * skinning is assumed and the renderable will be added as-is, by reference.
	 * Otherwise the renderable will be merged with other renderables as much as
	 * possible, depending on the {@link Mesh#getVertexAttributes()},
	 * {@link Renderable#material} and primitiveType (in that order). The
	 * {@link Renderable#environment}, {@link Renderable#shader} and
	 * {@link Renderable#userData} values (if any) are removed.
	 * 
	 * @param renderable The {@link Renderable} to add, should not change while the
	 *                   cache is needed.
	 */
