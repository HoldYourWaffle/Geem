...
/**
 * A {@link ParticleControllerRenderer} which will render particles as
 * {@link ModelInstance} to a {@link ModelInstanceParticleBatch}.
 * 
 * @author Inferno
 */
	...
	public void update() {
		for (int i = 0, positionOffset = 0, c = controller.particles.size; i < c; ++i, positionOffset += renderData.positionChannel.strideSize) {
			...
			if (hasColor) {
				...
				BlendingAttribute blendingAttribute = (BlendingAttribute) instance.materials.get(0)
				...
				if (blendingAttribute != null)
					blendingAttribute.opacity = renderData.colorChannel.data[colorOffset
							+ ParticleChannels.AlphaOffset];
			}
		}
		super.update();
	}
