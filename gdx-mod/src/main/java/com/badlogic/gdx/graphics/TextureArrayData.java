...
/**
 * Used by a {@link TextureArray} to load the pixel data. The TextureArray will
 * request the TextureArrayData to prepare itself through {@link #prepare()} and
 * upload its data using {@link #consumeTextureArrayData()}. These are the first
 * methods to be called by TextureArray. After that the TextureArray will invoke
 * the other methods to find out about the size of the image data, the format,
 * whether the TextureArrayData is able to manage the pixel data if the OpenGL
 * ES context is lost.
 * </p>
 *
 * Before a call to either {@link #consumeTextureArrayData()}, TextureArray will
 * bind the OpenGL ES texture.
 * </p>
 *
 * Look at {@link FileTextureArrayData} for example implementation of this
 * interface.
 * 
 * @author Tomski
 */
public interface TextureArrayData {
	 ...
	 * {@link #prepare()} must preceed a call to this method. Any internal data
