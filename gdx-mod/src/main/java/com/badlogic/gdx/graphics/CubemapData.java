...
/**
 * Used by a {@link Cubemap} to load the pixel data. The Cubemap will request
 * the CubemapData to prepare itself through {@link #prepare()} and upload its
 * data using {@link #consumeCubemapData()}. These are the first methods to be
 * called by Cubemap. After that the Cubemap will invoke the other methods to
 * find out about the size of the image data, the format, whether the
 * CubemapData is able to manage the pixel data if the OpenGL ES context is
 * lost.
 * </p>
 * 
 * Before a call to either {@link #consumeCubemapData()}, Cubemap will bind the
 * OpenGL ES texture.
 * </p>
 * 
