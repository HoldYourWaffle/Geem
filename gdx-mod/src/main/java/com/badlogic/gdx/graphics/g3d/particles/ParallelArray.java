...
/**
 * This class represents an group of elements like an array, but the properties
 * of the elements are stored as separate arrays. These arrays are called
 * {@link Channel} and are represented by {@link ChannelDescriptor}. It's not
 * necessary to store primitive types in the channels but doing so will
 * "exploit" data locality in the JVM, which is ensured for primitive types. Use
 * {@link FloatChannel}, {@link IntChannel}, {@link ObjectChannel} to store the
 * data.
 * 
 * @author inferno
 */
public class ParallelArray {
	...
	public int capacity;
	/**
	 * the current amount of defined elements, do not change manually unless you
	 ...
	 * Adds and returns a channel described by the channel descriptor parameter. If
	 * a channel with the same id already exists, no allocation is performed and
	 * that channel is returned. Otherwise a new channel is allocated and
	 ...
	 * the given channel at position n
	 */
