...
/**
 * A 3x3 grid of texture regions. Any of the regions may be omitted. Padding may
 * be set as a hint on how to inset content on top of the ninepatch (by default
 * the eight "edge" textures of the nine-patch define the padding). When drawn
 * the eight "edge" patches will not be scaled, only the interior patch will be
 * scaled.
 * 
 * <p>
 * <b>NOTE</b>: This class expects a "post-processed" nine-patch, and not a raw
 * ".9.png" texture. That is, the textures given to this class should
 * <em>not</em> include the meta-data pixels from a ".9.png" that describe the
 * layout of the ninepatch over the interior of the graphic. That information
 * should be passed into the constructor either implicitly as the size of the
 * individual patch textures, or via the <code>left, right, top, bottom</code>
 * parameters to {@link #NinePatch(Texture, int, int, int, int)} or
 * {@link #NinePatch(TextureRegion, int, int, int, int)}.
 * 
 * <p>
 * A correctly created {@link TextureAtlas} is one way to generate a
 * post-processed nine-patch from a ".9.png" file.
 */
public class NinePatch {
	 ...
	 * the same width, etc). Patches may be <code>null</code>. Patch indices are
	 * specified via the public members {@link #TOP_LEFT}, {@link #TOP_CENTER}, etc.
	 */
	...
	private int add(TextureRegion region, float color, boolean isStretchW, boolean isStretchH) {
		...
		// center
		// of the texel where the neighboring pixel has 0% contribution in linear
		// blending mode.
	 ...
	 * {@link NinePatch#draw(Batch, float, float, float, float) draw} time. Default
	 * is {@link Color#WHITE}.
	 */
