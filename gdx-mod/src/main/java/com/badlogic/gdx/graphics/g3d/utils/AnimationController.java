 ...
 * Use the {@link #setAnimation(String, int, float, AnimationListener)} method
 * to change the current animation. Use the
 * {@link #animate(String, int, float, AnimationListener, float)} method to
 * start an animation, optionally blending onto the current animation. Use the
 * {@link #queue(String, int, float, AnimationListener, float)} method to queue
 * an animation to be played when the current animation is finished. Use the
 * {@link #action(String, int, float, AnimationListener, float)} method to play
 * a (short) animation on top of the current animation.
 * 
 * You can use multiple AnimationControllers on the same ModelInstance, as long
 * as they don't interfere with each other (don't affect the same
 * {@link Node}s).
 * 
 * @author Xoppa
 */
public class AnimationController extends BaseAnimationController {
	...
	/**
	 * The animation queued to be played when the {@link #current} animation is
	 * completed. Do not alter this value.
	 */
	public AnimationDesc queued;
	/**
	 * The transition time which should be applied to the queued animation. Do not
	 * alter this value.
	 */
	...
	/**
	 * Whether to allow the same animation to be played while playing that
	 * animation.
	 */
	 ...
	 * 
	 * @param target The {@link ModelInstance} on which the animations will be
	 *               performed.
	 */
