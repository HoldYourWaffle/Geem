...
/**
 * Class for encoding and decoding ETC1 compressed images. Also provides methods
 * to add a PKM header.
 * 
 * @author mzechner
 */
public class ETC1 {
	...
	public final static class ETC1Data implements Disposable {
		...
		/**
		 * the offset in bytes to the actual compressed data. Might be 16 if this
	 ...
	 */
	public static native int getCompressedDataSize(int width,
			int height); /*
	 ...
	 * @param height the height in pixels
	 */
	public static native void formatHeader(ByteBuffer header, int offset, int width,
			int height); /*
	 ...
	 */
	static native int getWidthPKM(ByteBuffer header,
			int offset); /*
	 ...
	 */
	static native int getHeightPKM(ByteBuffer header,
			int offset); /*
	 ...
	 * @return the width stored in the PKM header
	 */
	static native boolean isValidPKM(ByteBuffer header,
			int offset); /*
	 ...
	 * 
	 * @param compressedData the compressed image data in a direct native order
	 *                       {@link ByteBuffer}
	 * @param offset         the offset in bytes to the image data from the start of
	 *                       the buffer
	 * @param decodedData    the decoded data in a direct native order ByteBuffer,
	 *                       must hold width * height * pixelSize bytes.
	 ...
	 * @param offset    the offset in bytes to the image data from the start of the
	 *                  buffer
	 * @param width     the width in pixels
	 * @param height    the height in pixels
	 * @param pixelSize the pixel size, either 2 (RGB565) or 3 (RGB888)
	 * @return a new direct native order ByteBuffer containing the compressed image
	 *         data
	 */
	private static native ByteBuffer encodeImage(ByteBuffer imageData, int offset, int width, int height,
			int pixelSize); /*
							 * int compressedSize = etc1_get_encoded_data_size(width, height); etc1_byte*
							 * compressedData = (etc1_byte*)malloc(compressedSize);
							 * etc1_encode_image((etc1_byte*)imageData + offset, width, height, pixelSize,
	...
	private static native ByteBuffer encodeImagePKM(ByteBuffer imageData, int offset, int width, int height,
			int pixelSize); /*
							 * int compressedSize = etc1_get_encoded_data_size(width, height); etc1_byte*
							 * compressed = (etc1_byte*)malloc(compressedSize + ETC_PKM_HEADER_SIZE);
							 * etc1_pkm_format_header(compressed, width, height);
							 * etc1_encode_image((etc1_byte*)imageData + offset, width, height, pixelSize,
							 * width * pixelSize, compressed + ETC_PKM_HEADER_SIZE); return
							 * env->NewDirectByteBuffer(compressed, compressedSize + ETC_PKM_HEADER_SIZE);
							 */
}
