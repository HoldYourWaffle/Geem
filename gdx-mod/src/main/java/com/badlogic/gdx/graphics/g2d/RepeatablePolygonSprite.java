...
public class RepeatablePolygonSprite {
	 ...
	 * bulidVertices method, it can be figured out if points is on the wall of it's
	 * own grid box or not, to set u/v properly. Any other implementations are
	 * welcome
	 */
		...
		for (int i = 0; i < parts.size; i++) {
			...
			for (int j = 0; j < verts.length; j += 2) {
				...
				if (verts[j] == col * gridWidth)
					u = 0f;
				if (verts[j] == (col + 1) * gridWidth)
					u = 1f;
				if (verts[j + 1] == row * gridHeight)
					v = 0f;
				if (verts[j + 1] == (row + 1) * gridHeight)
					v = 1f;
	...
	public void draw(PolygonSpriteBatch batch) {
		...
		for (int i = 0; i < vertices.size; i++) {
			batch.draw(region.getTexture(), vertices.get(i), 0, vertices.get(i).length, indices.get(i), 0,
