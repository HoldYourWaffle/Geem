...
/**
 * This class is used by particle batches to sort the particles before
 * rendering.
 * 
 * @author Inferno
 */
public abstract class ParticleSorter {
	...
	public static class Distance extends ParticleSorter {
		...
		public <T extends ParticleControllerRenderData> int[] sort(Array<T> renderData) {
			...
			for (ParticleControllerRenderData data : renderData) {
				for (int k = 0, c = i
						+ data.controller.particles.size; i < c; ++i, k += data.positionChannel.strideSize) {
	 ...
	 * the particles (i.e increase backing array size)
	 */
	public void ensureCapacity(int capacity) {
	}
}
