...
public class PatchShapeBuilder extends BaseShapeBuilder {
	...
	public static void build(MeshPartBuilder builder, VertexInfo corner00, VertexInfo corner10, VertexInfo corner11,
		...
		for (int u = 0; u <= divisionsU; u++) {
			...
			for (int v = 0; v <= divisionsV; v++) {
				...
				if (u > 0 && v > 0)
					builder.rect((short) (idx - divisionsV - 2), (short) (idx - 1), idx,
	 ...
	 * type.
	 */
	public static void build(MeshPartBuilder builder, Vector3 corner00, Vector3 corner10, Vector3 corner11,
			Vector3 corner01, Vector3 normal, int divisionsU, int divisionsV) {
	 ...
	 * type.
	 */
	public static void build(MeshPartBuilder builder, float x00, float y00, float z00, float x10, float y10, float z10,
			float x11, float y11, float z11, float x01, float y01, float z01, float normalX, float normalY,
			float normalZ, int divisionsU, int divisionsV) {
		build(builder, vertTmp1.set(null).setPos(x00, y00, z00).setNor(normalX, normalY, normalZ).setUV(0f, 1f),
				...
				vertTmp4.set(null).setPos(x01, y01, z01).setNor(normalX, normalY, normalZ).setUV(0f, 0f), divisionsU,
				divisionsV);
	}
}
