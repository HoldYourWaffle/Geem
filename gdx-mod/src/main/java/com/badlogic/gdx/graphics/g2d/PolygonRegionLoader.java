...
/**
 * loads {@link PolygonRegion PolygonRegions} using a
 * {@link com.badlogic.gdx.graphics.g2d.PolygonRegionLoader}
 * 
 * @author dermetfan
 */
public class PolygonRegionLoader extends SynchronousAssetLoader<PolygonRegion, PolygonRegionParameters> {
	 ...
	 * {@link AssetDescriptor} for the file referenced on that line will be added to
	 * the returned Array. Otherwise a sibling of the given file with the same name
	 * and the first found extension in
	 * {@link PolygonRegionParameters#textureExtensions params.textureExtensions}
	 * will be used. If no suitable file is found, the returned Array will be empty.
	 */
	@Override
	 ...
	 * s 200.0, 100.0, ...
	 * <p>
	 * Lines not prefixed with "s" are ignored. PSH files can be created with
	 * external tools, eg: <br>
		...
		try {
			while (true) {
				...
				if (line.startsWith("s")) {
					...
					// It would probably be better if PSH stored the vertices and triangles, then we
					// don't have to triangulate here.
					return new PolygonRegion(textureRegion, vertices,
