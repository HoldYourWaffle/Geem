...
public class BillboardParticleBatch extends BufferedParticleBatch<BillboardControllerRenderData> {
	protected static final Vector3 TMP_V1 = new Vector3(), TMP_V2 = new Vector3(), TMP_V3 = new Vector3(),
			TMP_V4 = new Vector3(), TMP_V5 = new Vector3(), TMP_V6 = new Vector3();
	...
	private static final VertexAttributes GPU_ATTRIBUTES = new VertexAttributes(
			...
			/*
			 * GPU_EXT_ATTRIBUTES = new VertexAttributes(new VertexAttribute(Usage.Position,
			 * 3, ShaderProgram.POSITION_ATTRIBUTE), new
			 * VertexAttribute(Usage.TextureCoordinates, 2,
			 * ShaderProgram.TEXCOORD_ATTRIBUTE+"0"), new VertexAttribute(Usage.Color, 4,
			 * ShaderProgram.COLOR_ATTRIBUTE), new VertexAttribute(sizeAndRotationUsage, 4,
			 * "a_sizeAndRotation"), new VertexAttribute(directionUsage, 3, "a_direction")),
			 */
			CPU_ATTRIBUTES = new VertexAttributes(
					new VertexAttribute(Usage.Position, 3, ShaderProgram.POSITION_ATTRIBUTE),
					new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0"),
					new VertexAttribute(Usage.ColorUnpacked, 4, ShaderProgram.COLOR_ATTRIBUTE));
	...
	private static final int GPU_POSITION_OFFSET = (short) (GPU_ATTRIBUTES.findByUsage(Usage.Position).offset / 4),
			...
			// Ext
			/*
			 * GPU_EXT_POSITION_OFFSET =
			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(Usage.Position).offset/4),
			 * GPU_EXT_UV_OFFSET =
			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(Usage.TextureCoordinates).offset/4),
			 * GPU_EXT_SIZE_ROTATION_OFFSET =
			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(sizeAndRotationUsage).offset/4),
			 * GPU_EXT_COLOR_OFFSET =
			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(Usage.Color).offset/4),
			 * GPU_EXT_DIRECTION_OFFSET =
			 * (short)(GPU_EXT_ATTRIBUTES.findByUsage(directionUsage).offset/4),
	...
	protected Renderable allocRenderable() {
		...
		renderable.meshPart.mesh = new Mesh(false, MAX_VERTICES_PER_MESH, MAX_PARTICLES_PER_MESH * 6,
				currentAttributes);
	...
	public void setVertexData() {
		if (useGPU) {
			...
			/*
			 * if(mode == AlignMode.ParticleDirection){ currentAttributes =
			 * GPU_EXT_ATTRIBUTES; currentVertexSize = GPU_EXT_VERTEX_SIZE; } else{
			 * currentAttributes = GPU_ATTRIBUTES; currentVertexSize = GPU_VERTEX_SIZE; }
			 */
		} else {
	...
	/**
	 * Sets the current align mode. It will reallocate internal data, use only when
	 * necessary.
	 */
	...
	/*
	 * //Required + Color + Rotation + Direction private static void putVertex(
	 * float[] vertices, int offset, float x, float y, float z, float u, float v,
	 * float scaleX, float scaleY, float cosRotation, float sinRotation, float r,
	 * float g, float b, float a, Vector3 direction) { //Position vertices[offset +
	 * GPU_EXT_POSITION_OFFSET] = x; vertices[offset + GPU_EXT_POSITION_OFFSET+1] =
	 * y; vertices[offset + GPU_EXT_POSITION_OFFSET+2] = z; //UV vertices[offset +
	 * GPU_EXT_UV_OFFSET] = u; vertices[offset + GPU_EXT_UV_OFFSET+1] = v; //Scale
	 * vertices[offset + GPU_EXT_SIZE_ROTATION_OFFSET] = scaleX; vertices[offset +
	 * GPU_EXT_SIZE_ROTATION_OFFSET+1] = scaleY; vertices[offset +
	 * GPU_EXT_SIZE_ROTATION_OFFSET+2] = cosRotation; vertices[offset +
	 * GPU_EXT_SIZE_ROTATION_OFFSET+3] = sinRotation; //Color vertices[offset +
	 * GPU_EXT_COLOR_OFFSET] = r; vertices[offset + GPU_EXT_COLOR_OFFSET+1] = g;
	 * vertices[offset + GPU_EXT_COLOR_OFFSET+2] = b; vertices[offset +
	 * GPU_EXT_COLOR_OFFSET+3] = a; //Direction vertices[offset +
	 * GPU_EXT_DIRECTION_OFFSET] = direction.x; vertices[offset +
	 * GPU_EXT_DIRECTION_OFFSET +1] = direction.y; vertices[offset +
	 * GPU_EXT_DIRECTION_OFFSET +2] = direction.z; }
	 */
	
	// CPU
	// Required
	private static void putVertex(float[] vertices, int offset, Vector3 p, float u, float v, float r, float g, float b,
			float a) {
		// Position
	...
	/*
	 * private void fillVerticesToParticleDirectionGPU (int[] particlesOffset) { int
	 * tp=0; for(BillboardControllerRenderData data : renderData){ FloatChannel
	 * scaleChannel = data.scaleChannel; FloatChannel regionChannel =
	 * data.regionChannel; FloatChannel positionChannel = data.positionChannel;
	 * FloatChannel colorChannel = data.colorChannel; FloatChannel rotationChannel =
	 * data.rotationChannel;
	 * 
	 * for(int p=0, c = data.controller.particles.size; p < c; ++p, ++tp){ int
	 * baseOffset = particlesOffset[tp]*currentVertexSize*4; float scale =
	 * scaleChannel.data[p* scaleChannel.strideSize]; int regionOffset =
	 * p*regionChannel.strideSize; int positionOffset =
	 * p*positionChannel.strideSize; int colorOffset = p*colorChannel.strideSize;
	 * int rotationOffset = p*rotationChannel.strideSize; int velocityOffset = p*
	 * velocityChannel.strideSize; float px = positionChannel.data[positionOffset +
	 * ParticleChannels.XOffset], py = positionChannel.data[positionOffset +
	 * ParticleChannels.YOffset], pz = positionChannel.data[positionOffset +
	 * ParticleChannels.ZOffset]; float u = regionChannel.data[regionOffset
	 * +ParticleChannels.UOffset]; float v = regionChannel.data[regionOffset
	 * +ParticleChannels.VOffset]; float u2 = regionChannel.data[regionOffset
	 * +ParticleChannels.U2Offset]; float v2 = regionChannel.data[regionOffset
	 * +ParticleChannels.V2Offset]; float sx = regionChannel.data[regionOffset
	 * +ParticleChannels.HalfWidthOffset] * scale, sy =
	 * regionChannel.data[regionOffset+ParticleChannels.HalfHeightOffset] * scale;
	 * float r = colorChannel.data[colorOffset +ParticleChannels.RedOffset]; float g
	 * = colorChannel.data[colorOffset +ParticleChannels.GreenOffset]; float b =
	 * colorChannel.data[colorOffset +ParticleChannels.BlueOffset]; float a =
	 * colorChannel.data[colorOffset +ParticleChannels.AlphaOffset]; float
	 * cosRotation = rotationChannel.data[rotationOffset
	 * +ParticleChannels.CosineOffset]; float sinRotation =
	 * rotationChannel.data[rotationOffset +ParticleChannels.SineOffset]; float vx =
	 * velocityChannel.data[velocityOffset + ParticleChannels.XOffset], vy =
	 * velocityChannel.data[velocityOffset + ParticleChannels.YOffset], vz =
	 * velocityChannel.data[velocityOffset + ParticleChannels.ZOffset];
	 * 
	 * //bottom left, bottom right, top right, top left TMP_V1.set(vx, vy,
	 * vz).nor(); putVertex(vertices, baseOffset, px, py, pz, u, v2, -sx, -sy,
	 * cosRotation, sinRotation, r, g, b, a, TMP_V1); baseOffset +=
	 * currentVertexSize; putVertex(vertices, baseOffset, px, py, pz, u2, v2, sx,
	 * -sy, cosRotation, sinRotation, r, g, b, a, TMP_V1); baseOffset +=
	 * currentVertexSize; putVertex(vertices, baseOffset, px, py, pz, u2, v, sx, sy,
	 * cosRotation, sinRotation, r, g, b, a, TMP_V1); baseOffset +=
	 * currentVertexSize; putVertex(vertices, baseOffset, px, py, pz, u, v, -sx, sy,
	 * cosRotation, sinRotation, r, g, b, a, TMP_V1); } } }
	 * 
	 * private void fillVerticesToParticleDirectionCPU (int[] particlesOffset) { int
	 * tp=0; for(ParticleController controller : renderData){ FloatChannel
	 * scaleChannel = controller.particles.getChannel(ParticleChannels.Scale);
	 * FloatChannel regionChannel =
	 * controller.particles.getChannel(ParticleChannels.TextureRegion); FloatChannel
	 * positionChannel = controller.particles.getChannel(ParticleChannels.Position);
	 * FloatChannel colorChannel =
	 * controller.particles.getChannel(ParticleChannels.Color); FloatChannel
	 * rotationChannel =
	 * controller.particles.getChannel(ParticleChannels.Rotation2D); FloatChannel
	 * velocityChannel =
	 * controller.particles.getChannel(ParticleChannels.Accelleration);
	 * 
	 * for(int p=0, c = controller.particles.size; p < c; ++p, ++tp){ int baseOffset
	 * = particlesOffset[tp]*currentVertexSize*4; float scale = scaleChannel.data[p*
	 * scaleChannel.strideSize]; int regionOffset = p*regionChannel.strideSize; int
	 * positionOffset = p*positionChannel.strideSize; int colorOffset =
	 * p*colorChannel.strideSize; int rotationOffset = p*rotationChannel.strideSize;
	 * int velocityOffset = p* velocityChannel.strideSize; float px =
	 * positionChannel.data[positionOffset + ParticleChannels.XOffset], py =
	 * positionChannel.data[positionOffset + ParticleChannels.YOffset], pz =
	 * positionChannel.data[positionOffset + ParticleChannels.ZOffset]; float u =
	 * regionChannel.data[regionOffset +ParticleChannels.UOffset]; float v =
	 * regionChannel.data[regionOffset +ParticleChannels.VOffset]; float u2 =
	 * regionChannel.data[regionOffset +ParticleChannels.U2Offset]; float v2 =
	 * regionChannel.data[regionOffset +ParticleChannels.V2Offset]; float sx =
	 * regionChannel.data[regionOffset +ParticleChannels.HalfWidthOffset] * scale,
	 * sy = regionChannel.data[regionOffset+ParticleChannels.HalfHeightOffset] *
	 * scale; float r = colorChannel.data[colorOffset +ParticleChannels.RedOffset];
	 * float g = colorChannel.data[colorOffset +ParticleChannels.GreenOffset]; float
	 * b = colorChannel.data[colorOffset +ParticleChannels.BlueOffset]; float a =
	 * colorChannel.data[colorOffset +ParticleChannels.AlphaOffset]; float
	 * cosRotation = rotationChannel.data[rotationOffset
	 * +ParticleChannels.CosineOffset]; float sinRotation =
	 * rotationChannel.data[rotationOffset +ParticleChannels.SineOffset]; float vx =
	 * velocityChannel.data[velocityOffset + ParticleChannels.XOffset], vy =
	 * velocityChannel.data[velocityOffset + ParticleChannels.YOffset], vz =
	 * velocityChannel.data[velocityOffset + ParticleChannels.ZOffset]; Vector3 up =
	 * TMP_V1.set(vx,vy,vz).nor(), look =
	 * TMP_V3.set(camera.position).sub(px,py,pz).nor(), //normal right =
	 * TMP_V2.set(up).crs(look).nor(); //tangent look.set(right).crs(up).nor();
	 * right.scl(sx); up.scl(sy);
	 * 
	 * if(cosRotation != 1){ TMP_M3.setToRotation(look, cosRotation, sinRotation);
	 * putVertex(vertices, baseOffset, TMP_V6.set(-TMP_V1.x-TMP_V2.x,
	 * -TMP_V1.y-TMP_V2.y, -TMP_V1.z-TMP_V2.z).mul(TMP_M3).add(px, py, pz), u, v2,
	 * r, g, b, a); baseOffset += currentVertexSize; putVertex(vertices,
	 * baseOffset,TMP_V6.set(TMP_V1.x-TMP_V2.x, TMP_V1.y-TMP_V2.y,
	 * TMP_V1.z-TMP_V2.z).mul(TMP_M3).add(px, py, pz), u2, v2, r, g, b, a);
	 * baseOffset += currentVertexSize; putVertex(vertices,
	 * baseOffset,TMP_V6.set(TMP_V1.x+TMP_V2.x, TMP_V1.y+TMP_V2.y,
	 * TMP_V1.z+TMP_V2.z).mul(TMP_M3).add(px, py, pz), u2, v, r, g, b, a);
	 * baseOffset += currentVertexSize; putVertex(vertices, baseOffset,
	 * TMP_V6.set(-TMP_V1.x+TMP_V2.x, -TMP_V1.y+TMP_V2.y,
	 * -TMP_V1.z+TMP_V2.z).mul(TMP_M3).add(px, py, pz), u, v, r, g, b, a); } else {
	 * putVertex(vertices, baseOffset,TMP_V6.set(-TMP_V1.x-TMP_V2.x+px,
	 * -TMP_V1.y-TMP_V2.y+py, -TMP_V1.z-TMP_V2.z+pz), u, v2, r, g, b, a); baseOffset
	 * += currentVertexSize; putVertex(vertices,
	 * baseOffset,TMP_V6.set(TMP_V1.x-TMP_V2.x+px, TMP_V1.y-TMP_V2.y+py,
	 * TMP_V1.z-TMP_V2.z+pz), u2, v2, r, g, b, a); baseOffset += currentVertexSize;
	 * putVertex(vertices, baseOffset,TMP_V6.set(TMP_V1.x+TMP_V2.x+px,
	 * TMP_V1.y+TMP_V2.y+py, TMP_V1.z+TMP_V2.z+pz), u2, v, r, g, b, a); baseOffset
	 * += currentVertexSize; putVertex(vertices, baseOffset,
	 * TMP_V6.set(-TMP_V1.x+TMP_V2.x+px, -TMP_V1.y+TMP_V2.y+py,
			...
			for (int p = 0, c = data.controller.particles.size; p < c; ++p, ++tp) {
				...
				if (cosRotation != 1) {
					...
					putVertex(vertices, baseOffset,
							TMP_V6.set(-TMP_V1.x - TMP_V2.x, -TMP_V1.y - TMP_V2.y, -TMP_V1.z - TMP_V2.z).mul(TMP_M3)
									.add(px, py, pz),
							u, v2, r, g, b, a);
					baseOffset += currentVertexSize;
					putVertex(vertices, baseOffset,
					...
					putVertex(vertices, baseOffset,
							TMP_V6.set(-TMP_V1.x - TMP_V2.x + px, -TMP_V1.y - TMP_V2.y + py, -TMP_V1.z - TMP_V2.z + pz),
							u, v2, r, g, b, a);
					baseOffset += currentVertexSize;
					putVertex(vertices, baseOffset,
							TMP_V6.set(TMP_V1.x - TMP_V2.x + px, TMP_V1.y - TMP_V2.y + py, TMP_V1.z - TMP_V2.z + pz),
