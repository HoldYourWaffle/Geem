...
/**
 * Stores {@link GlyphRun runs} of glyphs for a piece of text. The text may
 * contain newlines and color markup tags.
 * 
 * @author Nathan Sweet
 * @author davebaol
...
public class GlyphLayout implements Poolable {
	 ...
	 * setText} with the whole string, the font's current color, and no alignment or
	 * wrapping.
	 */
	 ...
	 * Calls
	 * {@link #setText(BitmapFont, CharSequence, int, int, Color, float, int, boolean, String)
	 * setText} with the whole string and no truncation.
	 */
	...
	/**
	 * @param color       The default color to use for the text (the BitmapFont
	 *                    {@link BitmapFont#getColor() color} is not used). If
	 *                    {@link BitmapFontData#markupEnabled} is true, color markup
	 *                    tags in the specified string may change the color for
	 *                    portions of the text.
	 * @param halign      Horizontal alignment of the text, see {@link Align}.
	 * @param targetWidth The width used for alignment, line wrapping, and
	 *                    truncation. May be zero if those features are not used.
	 * @param truncate    If not null and the width of the glyphs exceed
	 *                    targetWidth, the glyphs are truncated and the glyphs for
	 *                    the specified truncate string are placed at the end. Empty
	 *                    string can be used to truncate without adding glyphs.
					...
					for (int i = 2; i < n; i++) {
						...
						if (wrapIndex == 0) { // No wrap index, move entire run to next line.
							...
							for (int glyphCount = run.glyphs.size; wrapIndex < glyphCount; wrapIndex++)
								if (!fontData.isWhitespace((char) run.glyphs.get(wrapIndex).id))
									break;
							if (wrapIndex > 0) {
	...
	private GlyphRun wrap(BitmapFontData fontData, GlyphRun first, Pool<GlyphRun> glyphRunPool, int wrapIndex,
		...
		for (; firstEnd > 0; firstEnd--)
			if (!fontData.isWhitespace((char) glyphs2.get(firstEnd - 1).id))
				break;
		...
		// Reduce first run width by the wrapped glyphs that have contributed to the
		// width.
		for (int n = firstEnd + 1; widthIndex > n;)
			first.width -= xAdvances2.get(--widthIndex);

		// Copy wrapped glyphs and xAdvances to second run.
		// The second run will contain the remaining glyph data, so swap instances
		// rather than copying to reduce large allocations.
	...
	private void adjustLastGlyph(BitmapFontData fontData, GlyphRun run) {
		...
		run.width += width - run.xAdvances.peek(); // Can cause the run width to be > targetWidth, but the problem is
													// minimal.
	...
	private int parseColorMarkup(CharSequence str, int start, int end, Pool<Color> colorPool) {
		...
		switch (str.charAt(start)) {
		case '#':
			// Parse hex color RRGGBBAA where AA is optional and defaults to 0xFF if less
			// than 6 chars are used.
			int colorInt = 0;
	...
	static public class GlyphRun implements Poolable {
		...
		/**
		 * Contains glyphs.size+1 entries: First entry is X offset relative to the
		 * drawing position. Subsequent entries are the X advance relative to previous
		 * glyph position. Last entry is the width of the last glyph.
		 */
