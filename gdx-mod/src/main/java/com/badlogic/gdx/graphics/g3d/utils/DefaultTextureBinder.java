...
/**
 * Class that you assign a range of texture units and binds textures for you
 * within that range. It does some basic usage tracking to avoid unnecessary
 * bind calls.
 * 
 * @author xoppa
 */
public final class DefaultTextureBinder implements TextureBinder {
	...
	public void end() {
		/*
		 * No need to unbind and textures are set to null in begin() for(int i = 0; i <
		 * count; i++) { if (textures[i] != null) {
		 * Gdx.gl.glActiveTexture(GL20.GL_TEXTURE0 + offset + i);
		 * Gdx.gl.glBindTexture(GL20.GL_TEXTURE_2D, 0); textures[i] = null; } }
		 */
