 ...
 * variant is more performant and is preferred over vertex arrays if hardware
 * supports it.
 * </p>
 * 
 * <p>
...
public class Mesh implements Disposable {
	 ...
	 * 
	 * @param isStatic    whether this mesh is static or not. Allows for internal
	 *                    optimizations.
	 * @param maxVertices the maximum number of vertices this mesh can hold
	 * @param maxIndices  the maximum number of indices this mesh can hold
	 * @param attributes  the {@link VertexAttribute}s. Each vertex attribute
	 *                    defines one property of a vertex such as position, normal
	 ...
	 * @param maxVertices the maximum number of vertices this mesh can hold
	 * @param maxIndices  the maximum number of indices this mesh can hold
	 * @param attributes  the {@link VertexAttributes}. Each vertex attribute
	 *                    defines one property of a vertex such as position, normal
	 *                    or texture coordinate
	 */
	...
	/**
	 * Creates a new Mesh with the given attributes. Adds extra optimizations for
	 * dynamic (frequently modified) meshes.
	 * 
	 * @param staticVertices whether vertices of this mesh are static or not. Allows
	 *                       for internal optimizations.
	 * @param staticIndices  whether indices of this mesh are static or not. Allows
	 *                       for internal optimizations.
	 * @param maxVertices    the maximum number of vertices this mesh can hold
	 * @param maxIndices     the maximum number of indices this mesh can hold
	 * @param attributes     the {@link VertexAttributes}. Each vertex attribute
	 *                       defines one property of a vertex such as position,
	...
	/**
	 * Creates a new Mesh with the given attributes. This is an expert method with
	...
	/**
	 * Copies the vertices from the Mesh to the float array. The float array must be
	 * large enough to hold all the Mesh's vertices.
	 * 
	...
	/**
	 * Copies the the remaining vertices from the Mesh to the float array. The float
	 * array must be large enough to hold the remaining vertices.
	 * 
	 * @param srcOffset the offset (in number of floats) of the vertices in the mesh
	 *                  to copy
	 * @param vertices  the array to copy the vertices to
	 */
	 ...
	 * array must be large enough to hold count vertices.
	 * 
	 * @param srcOffset the offset (in number of floats) of the vertices in the mesh
	 *                  to copy
	 ...
	 * 
	 * @param srcOffset  the offset (in number of floats) of the vertices in the
	 *                   mesh to copy
	 * @param count      the amount of floats to copy
	 * @param vertices   the array to copy the vertices to
	 * @param destOffset the offset (in floats) in the vertices array to start
	 *                   copying
	 */
	...
	/**
	 * Copies the indices from the Mesh to the short array. The short array must be
	 * large enough to hold all the Mesh's indices.
	 * 
	...
	/**
	 * Copies the indices from the Mesh to the short array. The short array must be
	 * large enough to hold destOffset + all the Mesh's indices.
	 * 
	 * @param indices    the array to copy the indices to
	 ...
	 * array must be large enough to hold destOffset + all the remaining indices.
	 * 
	...
	/**
	 * Copies the indices from the Mesh to the short array. The short array must be
	 * large enough to hold destOffset + count indices.
	 * 
	 * @param srcOffset  the zero-based offset of the first index to fetch
	 ...
	 * {@link VertexBufferObject} automatically on a call to one of the render
	 * methods. Usually you want to use autobind. Manual binding is an expert
	 * functionality. There is a driver bug on the MSM720xa chips that will fuck up
	 * memory if you manipulate the vertices and indices of a Mesh multiple times
	 * while it is bound. Keep this in mind.
	 * 
	...
	/**
	 * Binds the underlying {@link VertexBufferObject} and {@link IndexBufferObject}
	 * if indices where given. Use this with OpenGL ES 2.0 and when auto-bind is
	 * disabled.
	 * 
	 ...
	 * If no indices are set then getNumVertices() / #vertices per primitive are
	 * rendered.
	 * </p>
	 * 
	 * <p>
	 ...
	 * into either the vertex buffer or the index buffer depending on whether
	 * indices are defined. count specifies the number of vertices or indices to use
	...
	/**
	 * Calculates the {@link BoundingBox} of the vertices contained in this mesh. In
	 * case no vertices are defined yet a {@link GdxRuntimeException} is thrown.
	...
	/**
	 * Calculates the radius of the bounding sphere around the specified center for
	 * the specified part.
	 * 
	 * @param centerX The X coordinate of the center of the bounding sphere
	 * @param centerY The Y coordinate of the center of the bounding sphere
	...
	/**
	 * @return the backing shortbuffer holding the indices. Does not have to be a
	 *         direct buffer on Android!
	 */
	...
	/**
	 * Invalidates all meshes so the next time they are rendered new VBO handles are
	 * generated.
	 * 
	 * @param app
	 */
	...
	/**
	 * Method to scale the positions in the mesh. Normals will be kept as is. This
	 * is a potentially slow operation, use with care. It will also create a
	...
	// TODO: Protected for now, because transforming a portion works but still
	// copies all vertices
	...
	/**
	 * Method to transform the positions in the float array. Normals will be kept as
	 * is. This is a potentially slow operation, use with care.
	 * 
	 * @param matrix     the transformation matrix
	...
	/**
	 * Method to transform the texture coordinates in the mesh. This is a
	 * potentially slow operation, use with care. It will also create a temporary
	 * float[] which will be garbage collected.
	 * 
	 * @param matrix the transformation matrix
	 */
	...
	/**
	 * Method to transform the texture coordinates (UV) in the float array. This is
	 * a potentially slow operation, use with care.
	 * 
	 * @param matrix     the transformation matrix
	 * @param vertices   the float array
	 * @param vertexSize the number of floats in each vertex
	...
	/**
	 * Copies this mesh optionally removing duplicate vertices and/or reducing the
	 * amount of attributes.
	 * 
	 * @param isStatic         whether the new mesh is static or not. Allows for
	 *                         internal optimizations.
	 * @param removeDuplicates whether to remove duplicate vertices if possible.
	 *                         Only the vertices specified by usage are checked.
	...
	public Mesh copy(boolean isStatic, boolean removeDuplicates, final int[] usage) {
		...
		if (usage != null) {
			...
			for (int element : usage)
				if (getVertexAttribute(element) != null) {
					size += getVertexAttribute(element).numComponents;
					as++;
				}
			if (size > 0) {
				...
				int ai = -1;
				for (int element : usage) {
					VertexAttribute a = getVertexAttribute(element);
					if (a == null)
						continue;
	 ...
	 * Copies this mesh.
	 * 
	 * @param isStatic whether the new mesh is static or not. Allows for internal
	 *                 optimizations.
	 * @return the copy of this mesh
	 */
