...
/**
 * Helper class to create {@link Model}s from code. To start building use the
 ...
 * {@link MeshPartBuilder} which can be used to build the node part.
 * 
 * @author Xoppa
 */
public class ModelBuilder {
	 ...
	 * 
	 * @return The newly created model. Call the {@link Model#dispose()} method when
	 *         no longer used.
	 */
	public Model end() {
		if (model == null)
	...
	public Model createCylinder(float width, float height, float depth, int divisions, int primitiveType,
		...
		begin();
		part("cylinder", primitiveType, attributes, material).cylinder(width, height, depth, divisions, angleFrom,
				angleTo);
		return end();
	}
	 ...
	 * will make the model responsible for disposing all referenced meshes.
	 */
	...
	public Model createArrow(float x1, float y1, float z1, float x2, float y2, float z2, float capLength,
		...
		begin();
		part("arrow", primitiveType, attributes, material).arrow(x1, y1, z1, x2, y2, z2, capLength, stemThickness,
				divisions);
		return end();
	}
