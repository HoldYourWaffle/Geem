...
public final class VertexAttributes implements Iterable<VertexAttribute>, Comparable<VertexAttributes> {
	...
	private int calculateOffsets() {
		int count = 0;
		for (VertexAttribute attribute2 : attributes) {
			VertexAttribute attribute = attribute2;
	...
	public int hashCode() {
		...
		for (VertexAttribute attribute : attributes)
			result = result * 61 + attribute.hashCode();
		return (int) (result ^ (result >> 32));
	}
	
	/**
	 * Calculates a mask based on the contained {@link VertexAttribute} instances.
	 * The mask is a bit-wise or of each attributes {@link VertexAttribute#usage}.
	 * 
	 * @return the mask
	 */
	public long getMask() {
	...
	/**
	 * Calculates the mask based on {@link VertexAttributes#getMask()} and packs the
	 * attributes count into the last 32 bits.
	 * 
