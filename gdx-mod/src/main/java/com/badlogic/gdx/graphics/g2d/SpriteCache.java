 ...
 * textures are cached and given an ID, which can later be used for drawing. The
 * size, color, and texture region for each cached image cannot be modified.
 * This information is stored in video memory and does not have to be sent to
 * the GPU each time it is drawn.<br>
 * <br>
 * To cache {@link Sprite sprites} or {@link Texture textures}, first call
 * {@link SpriteCache#beginCache()}, then call the appropriate add method to
 * define the images. To complete the cache, call {@link SpriteCache#endCache()}
 * and store the returned cache ID.<br>
 * <br>
 * To draw with SpriteCache, first call {@link #begin()}, then call
 * {@link #draw(int)} with a cache ID. When SpriteCache drawing is complete,
 * call {@link #end()}.<br>
 * <br>
 * By default, SpriteCache draws using screen coordinates and uses an x-axis
 * pointing to the right, an y-axis pointing upwards and the origin is the
 * bottom left corner of the screen. The default transformation and projection
 * matrices can be changed. If the screen is
 * {@link ApplicationListener#resize(int, int) resized}, the SpriteCache's
 * matrices must be updated. For example:<br>
 * <code>cache.getProjectionMatrix().setToOrtho2D(0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());</code><br>
 * <br>
 * Note that SpriteCache does not manage blending. You will need to enable
 * blending (<i>Gdx.gl.glEnable(GL10.GL_BLEND);</i>) and set the blend func as
 * needed before or between calls to {@link #draw(int)}.<br>
 * <br>
 * SpriteCache is managed. If the OpenGL context is lost and the restored, all
 * OpenGL resources a SpriteCache uses internally are restored.<br>
 * <br>
 * SpriteCache is a reasonably heavyweight object. Typically only one instance
 * should be used for an entire application.<br>
 * <br>
 * SpriteCache works with OpenGL ES 1.x and 2.0. For 2.0, it uses its own custom
 * shader to draw.<br>
 * <br>
...
public class SpriteCache implements Disposable {
	...
	/**
	 * Creates a cache with the specified size, using a default shader if OpenGL ES
	 * 2.0 is being used.
	 * 
	 * @param size       The maximum number of images this cache can hold. The
	 *                   memory required to hold the images is allocated up front.
	 ...
	 * called "a_texCoords", the color attribute is called "a_color". The projection
	 * matrix is uploaded via a mat4 uniform called "u_proj", the transform matrix
	 * is uploaded via a uniform called "u_trans", the combined transform and
	 * projection matrx is is uploaded via a mat4 uniform called "u_projTrans". The
