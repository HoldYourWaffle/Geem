...
/**
 * A PolygonSpriteBatch is used to draw 2D polygons that reference a texture
 ...
 * {@link PolygonSpriteBatch#begin()} method which will setup appropriate render
 * states. When you are done with drawing you have to call
 * {@link PolygonSpriteBatch#end()} which will actually draw the things you
 * specified.
 * <p>
 ...
 * so wish.
 * <p>
 * A PolygonSpriteBatch is managed. In case the OpenGL context is lost all
 * OpenGL resources a PolygonSpriteBatch uses internally get invalidated. A
 * context is lost when a user switches to another application or receives an
 * incoming call on Android. A SpritPolygonSpriteBatcheBatch will be
 * automatically reloaded after the OpenGL context is restored.
 * <p>
 * A PolygonSpriteBatch is a pretty heavy object so you should only ever have
 * one in your program.
 * <p>
 * A PolygonSpriteBatch works with OpenGL ES 1.x and 2.0. In the case of a 2.0
 * context it will use its own custom shader to draw all provided sprites. You
 * can set your own custom shader via {@link #setShader(ShaderProgram)}.
 * <p>
...
public class PolygonSpriteBatch implements Batch {
	...
	/**
	 * Constructs a PolygonSpriteBatch with the default shader, 2000 vertices, and
	 * 4000 triangles.
	 * 
	...
	/**
	 * Constructs a PolygonSpriteBatch with the default shader, size vertices, and
	 * size * 2 triangles.
	 * 
	 * @param size The max number of vertices and number of triangles in a single
	 *             batch. Max of 32767.
	 * @see #PolygonSpriteBatch(int, int, ShaderProgram)
	 */
	...
	/**
	 * Constructs a PolygonSpriteBatch with the specified shader, size vertices and
	 * size * 2 triangles.
	 * 
	 * @param size The max number of vertices and number of triangles in a single
	 *             batch. Max of 32767.
	...
	/**
	 * Constructs a new PolygonSpriteBatch. Sets the projection matrix to an
	 * orthographic projection with y-axis point upwards, x-axis point to the right
	 * and the origin being in the bottom left corner of the screen. The projection
	 * will be pixel perfect with respect to the current screen resolution.
	 * <p>
	 ...
	 * {@link SpriteBatch#createDefaultShader()}.
	 * 
	 * @param maxVertices   The max number of vertices in a single batch. Max of
	 *                      32767.
	 ...
	 *                      PolygonSpriteBatch and must be disposed separately. May
	 *                      be null to use the default shader.
	 */
	...
	/**
	 * Draws a polygon region with the bottom left corner at x,y having the width
	...
	/**
	 * Draws a polygon region with the bottom left corner at x,y and stretching the
	 ...
	 * by originX, originY relative to the origin. Scale specifies the scaling
	 * factor by which the polygon region should be scaled around originX, originY.
	 * Rotation specifies the angle of counter clockwise rotation of the rectangle
	...
	/**
	 * Draws the polygon using the given vertices and triangles. Each vertices must
	 * be made up of 5 elements in this order: x, y, color, u, v.
	 */
