 ...
 * in pixels as well as a {@link Format} specifying the number and order of
 * color components per pixel. Coordinates of pixels are specified with respect
 * to the top left corner of the image, with the x-axis pointing to the right
 ...
 * method will scale and stretch the source image to a target image. There
 * either nearest neighbour or bilinear filtering can be used.
 * </p>
 * 
 * <p>
 * A Pixmap stores its data in native heap memory. It is mandatory to call
 * {@link Pixmap#dispose()} when the pixmap is no longer needed, otherwise
...
public class Pixmap implements Disposable {
	...
	/**
	 * Sets the type of {@link Blending} to be used for all operations. Default is
	 * {@link Blending#SourceOver}.
	 * 
	...
	public void setFilter(Filter filter) {
		this.filter = filter;
		pixmap.setScale(
				filter == Filter.NearestNeighbour ? Gdx2DPixmap.GDX2D_SCALE_NEAREST : Gdx2DPixmap.GDX2D_SCALE_LINEAR);
	}
	...
	/**
	 * Creates a new Pixmap instance from the given encoded image data. The image
	 * can be encoded as JPEG, PNG or BMP.
	 * 
...
// public void setStrokeWidth (int width);
	 ...
	 * scale and stretch the source image to the specified target rectangle. Use
	 * {@link Pixmap#setFilter(Filter)} to specify the type of filtering to be used
	...
	/**
	 * Draws a circle outline with the center at x,y and a radius using the current
	...
	/**
	 * Fills a triangle with vertices at x1,y1 and x2,y2 and x3,y3 using the current
	 * color.
	 * 
	...
	/**
	 * Returns the 32-bit RGBA8888 value of the pixel at x, y. For Alpha formats the
	 * RGB components will be one.
	 * 
	 * @param x The x-coordinate
	 * @param y The y-coordinate
	 ...
	 * 
	 * @return one of GL_ALPHA, GL_RGB, GL_RGBA, GL_LUMINANCE, or
	 *         GL_LUMINANCE_ALPHA.
	 */
	 ...
	 * each value is encoded as a byte. For the format LuminanceAlpha the luminance
	 * is the first byte and the alpha is the second byte of the pixel. For the
	 * formats RGB888 and RGBA8888 the color components are stored in a single byte
	 * each in the order red, green, blue (alpha). For the formats RGB565 and
	 * RGBA4444 the pixel colors are stored in shorts in machine dependent order.
	 * 
