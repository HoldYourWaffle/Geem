 ...
 * <p>
 * By default a 2D orthographic projection with the origin in the lower left
 * corner is used and units are specified in screen pixels. This can be changed
 * by configuring the projection matrix, usually using the
 * {@link Camera#combined} matrix. If the screen resolution changes, the
 * projection matrix may need to be updated.
 * <p>
 * Shapes are rendered in batches to increase performance. Standard usage
 ...
 * </pre>
 * 
 * ShapeRenderer has a second matrix called the transformation matrix which is
 * used to rotate, scale and translate shapes in a more flexible manner. The
 * following example shows how to rotate a rectangle around its center using the
 * z-axis as the rotation axis and placing it's center at (20, 12, 2):
 * 
 * <pre>
 * shapeRenderer.begin(ShapeType.Line);
 ...
 * </pre>
 * 
 * Matrix operations all use postmultiplication and work just like glTranslate,
 * glScale and glRotate. The last transformation specified will be the first
 * that is applied to a shape (rotate then translate in the above example).
 * <p>
 * The projection and transformation matrices are a state of the ShapeRenderer,
 * just like the color, and will be applied to all shapes until they are
 * changed.
 * 
 * @author mzechner
 * @author stbachmann
...
public class ShapeRenderer implements Disposable {
	 ...
	 * type, the batch is flushed and the shape type is changed automatically. This
	 * can increase the number of batch flushes if care is not taken to draw the
	 * same type of shapes together. Default is false.
	 */
	...
	/**
	 * Starts a new batch of shapes. Shapes drawn within the batch will attempt to
	 * use the type specified. The call to this method must be paired with a call to
	 * {@link #end()}.
	 * 
	...
	/**
	 * Draws a line using {@link ShapeType#Line} or {@link ShapeType#Filled}. The
	 * line is drawn with two colors interpolated between the start and end points.
	 */
	...
	/**
	 * Draws a triangle in x/y plane using {@link ShapeType#Line} or
	 * {@link ShapeType#Filled}.
	 */
	 ...
	 * {@link ShapeType#Line} or {@link ShapeType#Filled}.
	 */
	...
	/**
	 * Draws a rectangle in the x/y plane using {@link ShapeType#Line} or
	 * {@link ShapeType#Filled}.
	 */
	...
	/**
	 * Draws a rectangle in the x/y plane using {@link ShapeType#Line} or
	 * {@link ShapeType#Filled}. The x and y specify the lower left corner.
	 * 
	...
	/**
	 * Draws a rectangle in the x/y plane using {@link ShapeType#Line} or
	 * {@link ShapeType#Filled}. The x and y specify the lower left corner. The
	 * originX and originY specify the point about which to rotate the rectangle.
	 */
	 ...
	 */
	public void rect(float x, float y, float originX, float originY, float width, float height, float scaleX,
			float scaleY, float degrees, Color col1, Color col2, Color col3, Color col4) {
		check(ShapeType.Line, ShapeType.Filled, 8);
	...
	/**
	 * Draws a line using a rotated rectangle, where with one edge is centered at
	 * x1, y1 and the opposite edge centered at x2, y2.
	 */
	...
	/**
	 * Draws a cube using {@link ShapeType#Line} or {@link ShapeType#Filled}. The x,
	 * y and z specify the bottom, left, front corner of the rectangle.
	 */
	...
	/**
	 * Draws two crossed lines using {@link ShapeType#Line} or
	 * {@link ShapeType#Filled}.
	 */
	...
	/**
	 * Calls {@link #ellipse(float, float, float, float, int)} by estimating the
	 * number of segments needed for a smooth ellipse.
	 */
	...
	/**
	 * Calls {@link #ellipse(float, float, float, float, float, int)} by estimating
	...
	/**
	 * Draws a polygon in the x/y plane using {@link ShapeType#Line}. The vertices
	 * must contain at least 3 points (6 floats x,y).
	 */
	...
	/**
	 * Draws a polyline in the x/y plane using {@link ShapeType#Line}. The vertices
	 * must contain at least 2 points (4 floats x,y).
	 */
