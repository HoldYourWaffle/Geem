...
/**
 * A model represents a 3D assets. It stores a hierarchy of nodes. A node has a
 * transform and optionally a graphical part in form of a {@link MeshPart} and
 * {@link Material}. Mesh parts reference subsets of vertices in one of the
 * meshes of the model. Animations can be applied to nodes, to modify their
 * transform (translation, rotation, scale) over time.
 * </p>
 * 
 * A model can be rendered by creating a {@link ModelInstance} from it. That
 * instance has an additional transform to position the model in the world, and
 * allows modification of materials and nodes without destroying the original
 * model. The original model is the owner of any meshes and textures, all
 * instances created from the model share these resources. Disposing the model
 * will automatically make all instances invalid!
 * </p>
 * 
 * A model is created from {@link ModelData}, which in turn is loaded by a
 * {@link ModelLoader}.
 * 
...
public class Model implements Disposable {
	 ...
	 * materials without having to traverse the nodes
	 **/
	...
	/**
	 * parts of meshes, used by nodes that have a graphical representation FIXME not
	 * sure if superfluous, stored in Nodes as well, could be useful to create
	 * bullet meshes
	 **/
	public final Array<MeshPart> meshParts = new Array();
	/**
	 * Array of disposable resources like textures or meshes the Model is
	 ...
	 * resources to be managed by this model.
	 */
	public Model() {
	}
	 ...
	 * 
	 * @param modelData       the {@link ModelData} got from e.g.
	 *                        {@link ModelLoader}
	 * @param textureProvider the {@link TextureProvider} to use for loading the
	 *                        textures
	 */
		...
		if (mtl.textures != null) {
			for (ModelTexture tex : mtl.textures) {
				...
				case ModelTexture.USAGE_BUMP:
					result.set(
							new TextureAttribute(TextureAttribute.Bump, descriptor, offsetU, offsetV, scaleU, scaleV));
					break;
				...
				case ModelTexture.USAGE_REFLECTION:
					result.set(new TextureAttribute(TextureAttribute.Reflection, descriptor, offsetU, offsetV, scaleU,
							scaleV));
					break;
				}
			}
