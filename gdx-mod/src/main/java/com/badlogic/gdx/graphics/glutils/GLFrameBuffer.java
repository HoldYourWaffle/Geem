 ...
 * You can get a hold of the gltexture by
 * {@link GLFrameBuffer#getColorBufferTexture()}. This class will only work with
 * OpenGL ES 2.0.
 * </p>
 *
 * <p>
...
public abstract class GLFrameBuffer<T extends GLTexture> implements Disposable {
	...
	/**
	 * Override this method in a derived class to set up the backing texture as you
	 * like.
	 */
	protected abstract T createTexture(FrameBufferTextureAttachmentSpec attachmentSpec);

	/**
	 * Override this method in a derived class to dispose the backing texture as you
	 * like.
	 */
	protected abstract void disposeColorTexture(T colorTexture);

	/**
	 * Override this method in a derived class to attach the backing texture to the
		...
		if (isMRT) {
			for (FrameBufferTextureAttachmentSpec attachmentSpec : bufferBuilder.textureAttachmentSpecs) {
				...
				if (attachmentSpec.isColorTexture()) {
					gl.glFramebufferTexture2D(GL20.GL_FRAMEBUFFER, GL20.GL_COLOR_ATTACHMENT0 + colorTextureCounter,
	...
	/**
	 * Binds the frame buffer and sets the viewport accordingly, so everything gets
	 * drawn to it.
	 */
	public void begin() {
		bind();
	...
	/**
	 * @return The OpenGL handle of the framebuffer (see
	 *         {@link GL20#glGenFramebuffer()})
	 */
	...
	/**
	 * @return The OpenGL handle of the (optional) depth buffer (see
	 *         {@link GL20#glGenRenderbuffer()}). May return 0 even if depth buffer
	 *         enabled
	 */
	...
	/**
	 * @return The OpenGL handle of the (optional) stencil buffer (see
	 *         {@link GL20#glGenRenderbuffer()}). May return 0 even if stencil
	 *         buffer enabled
	 */
	...
	/**
	 * @return The OpenGL handle of the packed depth & stencil buffer
	 *         (GL_DEPTH24_STENCIL8_OES) or 0 if not used.
	 **/
