...
public class DefaultShader extends BaseShader {
	...
	public static class Setters {
		...
		public final static Setter specularTexture = new LocalSetter() {
			...
			public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
				final int unit = shader.context.textureBinder.bind(
						((TextureAttribute) (combinedAttributes.get(TextureAttribute.Specular))).textureDescription);
				shader.set(inputID, unit);
			}
		};
		...
		public final static Setter emissiveTexture = new LocalSetter() {
			...
			public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
				final int unit = shader.context.textureBinder.bind(
						((TextureAttribute) (combinedAttributes.get(TextureAttribute.Emissive))).textureDescription);
				shader.set(inputID, unit);
			}
		};
		...
		public final static Setter reflectionTexture = new LocalSetter() {
			...
			public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
				final int unit = shader.context.textureBinder.bind(
						((TextureAttribute) (combinedAttributes.get(TextureAttribute.Reflection))).textureDescription);
				shader.set(inputID, unit);
			}
		};
			...
			public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
				if (renderable.environment == null)
					...
					if (combinedAttributes.has(ColorAttribute.AmbientLight))
						cacheAmbientCubemap
								.set(((ColorAttribute) combinedAttributes.get(ColorAttribute.AmbientLight)).color);
					...
					if (combinedAttributes.has(PointLightsAttribute.Type)) {
						Array<PointLight> lights = ((PointLightsAttribute) combinedAttributes
	...
	/**
	 * @deprecated Replaced by {@link Config#defaultCullFace} Set to 0 to disable
	 *             culling
	 */
	@Deprecated
	public static int defaultCullFace = GL20.GL_BACK;
	/**
	 * @deprecated Replaced by {@link Config#defaultDepthFunc} Set to 0 to disable
	 *             depth test
	 */
	@Deprecated
	...
	// TODO: Perhaps move responsibility for combining attributes to
	// RenderableProvider?
		...
		if (pointLightsLoc >= 0) {
			for (int i = 0; i < pointLights.length; i++) {
				...
				program.setUniformf(idx + pointLightsPositionOffset, pointLights[i].position.x,
						pointLights[i].position.y, pointLights[i].position.z);
