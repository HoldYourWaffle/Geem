...
/**
 * Wraps a standard OpenGL ES Cubemap. Must be disposed when it is no longer
 * used.
 * 
 * @author Xoppa
 */
public class Cubemap extends GLTexture {
	...
	public Cubemap(FileHandle positiveX, FileHandle negativeX, FileHandle positiveY, FileHandle negativeY,
		...
		this(TextureData.Factory.loadFromFile(positiveX, useMipMaps),
				TextureData.Factory.loadFromFile(negativeX, useMipMaps),
				TextureData.Factory.loadFromFile(positiveY, useMipMaps),
				TextureData.Factory.loadFromFile(negativeY, useMipMaps),
	...
	@Override
	public void dispose() {
		// this is a hack. reason: we have to set the glHandle to 0 for textures that
		// are
		// reloaded through the asset manager as we first remove (and thus dispose) the
		// texture
		// and then reload it. the glHandle is set to 0 in invalidateAllTextures prior
		// to
	 ...
	 * that a suitable {@link AssetLoader} is registered with the manager). Cubemaps
