 ...
 * OpenGL texture capabilities like texture compression, cubemapping,
 * mipmapping, etc.
 * 
 * For example, KTXTextureData can be used for {@link Texture} or
 * {@link Cubemap}.
 * 
...
public class KTXTextureData implements TextureData, CubemapData {
	...
	public void prepare() {
		...
		// We support normal ktx files as well as 'zktx' which are gzip ktx file with an
		// int length at the beginning (like ETC1).
		...
		if (endianTag != 0x04030201)
			compressedData.order(
					compressedData.order() == ByteOrder.BIG_ENDIAN ? ByteOrder.LITTLE_ENDIAN : ByteOrder.BIG_ENDIAN);
		...
		for (int level = 0; level < numberOfMipmapLevels; level++) {
			...
			for (int face = 0; face < numberOfFaces; face++) {
				...
				if (textureDimensions == 1) {
					// if (compressed)
					// Gdx.gl.glCompressedTexImage1D(target + face, level, glInternalFormat,
					// pixelWidth, 0, faceLodSize,
					// data);
					// else
					// Gdx.gl.glTexImage1D(target + face, level, glInternalFormat, pixelWidth, 0,
					...
					if (compressed) {
						if (glInternalFormat == ETC1.ETC1_RGB8_OES) {
							if (!Gdx.graphics.supportsExtension("GL_OES_compressed_ETC1_RGB8_texture")) {
								...
								Gdx.gl.glTexImage2D(target + face, level, pixmap.getGLInternalFormat(),
										pixmap.getWidth(), pixmap.getHeight(), 0, pixmap.getGLFormat(),
										pixmap.getGLType(), pixmap.getPixels());
								pixmap.dispose();
							} else {
								Gdx.gl.glCompressedTexImage2D(target + face, level, glInternalFormat, pixelWidth,
										pixelHeight, 0, faceLodSize, data);
							}
						} else {
						...
						Gdx.gl.glTexImage2D(target + face, level, glInternalFormat, pixelWidth, pixelHeight, 0,
								glFormat, glType, data);
				} else if (textureDimensions == 3) {
					if (numberOfArrayElements > 0)
						pixelDepth = numberOfArrayElements;
					// if (compressed)
					// Gdx.gl.glCompressedTexImage3D(target + face, level, glInternalFormat,
					// pixelWidth, pixelHeight, pixelDepth, 0,
					// faceLodSize, data);
					// else
					// Gdx.gl.glTexImage3D(target + face, level, glInternalFormat, pixelWidth,
					// pixelHeight, pixelDepth, 0, glFormat,
					// glType, data);
				}
			}
		}
