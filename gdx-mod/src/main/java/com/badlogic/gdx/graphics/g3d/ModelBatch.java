...
/**
 * Batches {@link Renderable} instances, fetches {@link Shader}s for them, sorts
 * them and then renders them. Fetching the shaders is done using a
 * {@link ShaderProvider}, which defaults to {@link DefaultShaderProvider}.
 * Sorting the renderables is done using a {@link RenderableSorter}, which
 * default to {@link DefaultRenderableSorter}.
 * 
 * The OpenGL context between the {@link #begin(Camera)} and {@link #end()} call
 * is maintained by the {@link RenderContext}.
 * 
 * To provide multiple {@link Renderable}s at once a {@link RenderableProvider}
 * can be used, e.g. a {@link ModelInstance}.
 * 
...
public class ModelBatch implements Disposable {
	 ...
	 * @param shaderProvider The {@link ShaderProvider} to use, will be disposed
	 *                       when this ModelBatch is disposed.
	...
	public ModelBatch(final RenderContext context, final ShaderProvider shaderProvider, final RenderableSorter sorter) {
		...
		this.context = (context == null) ? new RenderContext(new DefaultTextureBinder(DefaultTextureBinder.WEIGHTED, 1))
				: context;
	 ...
	 * custom ubershader. Requires OpenGL ES 2.0.
	 * 
	 ...
	 * {@link #end()}. The OpenGL context must not be altered between
	 ...
	 * {@link #setCamera(Camera)}, if you need to change the camera.
	 * 
	 * @return The current camera being used or null if called outside
	 *         {@link #begin(Camera)} and {@link #end()}.
	 */
	 ...
	 * RenderContext isn't owned by the ModelBatch, you are responsible for calling
	 * the {@link RenderContext#begin()} and {@link RenderContext#end()} methods
	 * yourself, as well as disposing the RenderContext.
	 * 
	 ...
	 * provided using one of the render() methods to be rendered. After a call to
	 * this method the OpenGL context can be altered again.
	 */
	public void end() {
		flush();
		if (ownContext)
	 ...
	 * Can only be called after a call to {@link #begin(Camera)} and before a call
	 * to {@link #end()}.
	 * 
	 ...
	 * Calls {@link RenderableProvider#getRenderables(Array, Pool)} and adds all
	 * returned {@link Renderable} instances to the current batch to be rendered.
	 * Any environment set on the returned renderables will be replaced with the
	 * given environment. Can only be called after a call to {@link #begin(Camera)}
	 ...
	 * Calls {@link RenderableProvider#getRenderables(Array, Pool)} and adds all
	 * returned {@link Renderable} instances to the current batch to be rendered.
	 * Any shaders set on the returned renderables will be replaced with the given
	 * {@link Shader}. Can only be called after a call to {@link #begin(Camera)} and
	 * before a call to {@link #end()}.
	 * 
	 * @param renderableProvider the renderable provider
	 ...
	 * returned {@link Renderable} instances to the current batch to be rendered.
	 * Any environment set on the returned renderables will be replaced with the
	 * given environment. Any shaders set on the returned renderables will be
	 * replaced with the given {@link Shader}. Can only be called after a call to
