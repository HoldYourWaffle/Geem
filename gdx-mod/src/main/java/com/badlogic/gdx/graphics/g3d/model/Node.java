...
/**
 * A node is part of a hierarchy of Nodes in a {@link Model}. A Node encodes a
 * transform relative to its parents. A Node can have child nodes. Optionally a
 * node can specify a {@link MeshPart} and a {@link Material} to be applied to
 * the mesh part.
 * 
 * @author badlogic
 */
public class Node {
	...
	public String id;
	/**
	 * Whether this node should inherit the transformation of its parent node,
	 * defaults to true. When this flag is false the value of
	 * {@link #globalTransform} will be the same as the value of
	 * {@link #localTransform} causing the transform to be independent of its parent
	 * transform.
	 */
	public boolean inheritTransform = true;
	/**
	 * Whether this node is currently being animated, if so the translation,
	 * rotation and scale values are not used.
	 */
	 ...
	 * ({@link #calculateLocalTransform()}) or any applied animation
	 **/
	public final Matrix4 localTransform = new Matrix4();
	/**
	 * the global transform, product of local transform and transform of the parent
	 ...
	 * children.
	 * 
	 * @param recursive whether to calculate the local/world transforms for
	 *                  children.
	 */
	 ...
	 * current parent. If the specified index is less than zero or equal or greater
	 * than {@link #getChildCount()} then the Node is added as the currently last
	 * child.
	 * 
	 ...
	 * return null). If the specified node currently isn't a child of this node then
	 * the removal is considered to be unsuccessful and the method will return
	 * false.
	 * 
	 ...
	 * {@link NodePart#copy()} method. Note that that method copies the material and
	 * nodes (bones) by reference. If you intend to use the copy in a different node
	 * tree (e.g. a different Model or ModelInstance) then you will need to update
	 * these references afterwards.
	 * 
	 * Override this method in your custom Node class to instantiate that class, in
	 * that case you should override the {@link #set(Node)} method as well.
	 */
	public Node copy() {
	 ...
	 * not attach it to the parent of node being copied. The {@link #parts} are
	 * copied using the {@link NodePart#copy()} method. Note that that method copies
	 * the material and nodes (bones) by reference. If you intend to use this node
	 * in a different node tree (e.g. a different Model or ModelInstance) then you
	 * will need to update these references afterwards.
	 * 
	 * Override this method in your custom Node class to copy any additional fields
	 * you've added.
	 * 
