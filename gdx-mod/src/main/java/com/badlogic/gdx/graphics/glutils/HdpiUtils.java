...
/**
 * To deal with HDPI monitors properly, use the glViewport and glScissor
 * functions of this class instead of directly calling OpenGL yourself. The
 * logical coordinate system provided by the operating system may not have the
 * same resolution as the actual drawing surface to which OpenGL draws, also
 * known as the backbuffer. This class will ensure, that you pass the correct
 * values to OpenGL for any function that expects backbuffer coordinates instead
...
public class HdpiUtils {
	/**
	 * Calls {@link GL20#glScissor(int, int, int, int)}, expecting the coordinates
	 * and sizes given in logical coordinates and automatically converts them to
	 * backbuffer coordinates, which may be bigger on HDPI screens.
	 */
