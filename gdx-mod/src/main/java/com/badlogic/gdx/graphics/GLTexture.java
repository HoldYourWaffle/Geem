 ...
 * its state like the TextureFilter and TextureWrap. Also provides some
 * (protected) static methods to create TextureData and upload image data.
 * 
...
public abstract class GLTexture implements Disposable {
	 ...
	 * @param u     the u wrap
	 * @param v     the v wrap
	 * @param force True to always set the values, even if they are the same as the
	 *              current values.
	 */
	 ...
	 * @param minFilter the minification filter
	 * @param magFilter the magnification filter
	 * @param force     True to always set the values, even if they are the same as
	 *                  the current values.
	 */
	...
	public static void uploadImageData(int target, TextureData data, int miplevel) {
		...
		if (data.useMipMaps()) {
			...
			Gdx.gl.glTexImage2D(target, miplevel, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(),
