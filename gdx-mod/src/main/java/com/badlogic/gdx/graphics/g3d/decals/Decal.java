...
/**
 * <p/>
 * Represents a sprite in 3d space. Typical 3d transformations such as
 * translation, rotation and scaling are supported. The position includes a z
 * component other than setting the depth no manual layering has to be
 * performed, correct overlay is guaranteed by using the depth buffer.
 * <p/>
...
public class Decal {
	...
	protected void transformVertices() {
		/**
		 * It would be possible to also load the x,y,z into a Vector3 and apply all the
		 * transformations using already existing methods. Especially the quaternion
		 * rotation already exists in the Quaternion class, it then would look like
		 * this:
		 * ----------------------------------------------------------------------------------------------------
		 * v3.set(vertices[xIndex] * scale.x, vertices[yIndex] * scale.y,
		 * vertices[zIndex]); rotation.transform(v3); v3.add(position); vertices[xIndex]
		 * = v3.x; vertices[yIndex] = v3.y; vertices[zIndex] = v3.z;
		 * ----------------------------------------------------------------------------------------------------
		 * However, a half ass benchmark with dozens of thousands decals showed that
		 * doing it "by hand", as done here, is about 10% faster. So while duplicate
		 * code should be avoided for maintenance reasons etc. the performance gain is
		 * worth it. The math doesn't change.
		 */
		float x, y, z, w;
		float tx, ty;
	 ...
	 * 
	 * @param textureRegion   Texture region to use
	 * @param hasTransparency Whether or not this sprite will be treated as having
	 ...
	 * @return Created decal
	 */
	// TODO : it would be convenient if {@link com.badlogic.gdx.graphics.Texture}
