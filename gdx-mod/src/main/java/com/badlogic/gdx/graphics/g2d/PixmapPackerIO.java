...
public class PixmapPackerIO {
	 ...
	 * as if it had been created using TexturePacker. Default {@link SaveParameters}
	 * will be used.
	 * 
	 * @param file   the file to which the atlas descriptor will be written, images
	 *               will be written as siblings
	 ...
	 * Saves the provided PixmapPacker to the provided file. The resulting file will
	 * use the standard TextureAtlas file format and can be loaded by TextureAtlas
	 * as if it had been created using TexturePacker.
	 * 
		...
		for (Page page : packer.pages) {
			if (page.rects.size > 0) {
				FileHandle pageFile = file
						.sibling(file.nameWithoutExtension() + "_" + (++index) + parameters.format.getExtension());
