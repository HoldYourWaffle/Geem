 ...
 * </p>
 * <p>
 * New objects are added using {@link DecalBatch#add(Decal)}, there is no limit
 * on how many decals can be added.<br/>
 * Once all the decals have been submitted a call to {@link DecalBatch#flush()}
 * will batch them together and send big chunks of geometry to the GL.
 * </p>
 * <p>
 * The size of the batch specifies the maximum number of decals that can be
 * batched together before they have to be submitted to the graphics pipeline.
 * The default size is {@link DecalBatch#DEFAULT_SIZE}. If it is known before
 * hand that not as many will be needed on average the batch can be downsized to
 * save memory. If the game is basically 3d based and decals will only be needed
 * for an orthogonal HUD it makes sense to tune the size down.
 * </p>
 * <p>
 * The way the batch handles things depends on the {@link GroupStrategy}.
 * Different strategies can be used to customize shaders, states, culling etc.
 * for more details see the {@link GroupStrategy} java doc.<br/>
 * While it shouldn't be necessary to change strategies, if you have to do so,
 * do it before calling {@link #add(Decal)}, and if you already did, call
 * {@link #flush()} first.
 * </p>
 */
public class DecalBatch implements Disposable {
	...
	/**
	 * Initializes the batch with the given amount of decal objects the buffer is
	 * able to hold when full.
	 * 
