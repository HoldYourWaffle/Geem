...
public class Gdx2DPixmap implements Disposable {
	...
	// @off
	/*
	 * JNI #include <gdx2d/gdx2d.h> #include <stdlib.h>
	 */

	private static native ByteBuffer load(long[] nativeData, byte[] buffer, int offset,
			int len); /*
						 * MANUAL const unsigned char* p_buffer = (const unsigned
						 * char*)env->GetPrimitiveArrayCritical(buffer, 0); gdx2d_pixmap* pixmap =
						 * gdx2d_load(p_buffer + offset, len);
						 ...
						 * p_native_data[0] = (jlong)pixmap; p_native_data[1] = pixmap->width;
						 * p_native_data[2] = pixmap->height; p_native_data[3] = pixmap->format;
						 * env->ReleasePrimitiveArrayCritical(nativeData, p_native_data, 0);
						 * 
						 * return pixel_buffer;
						 */

	private static native ByteBuffer newPixmap(long[] nativeData, int width, int height,
			int format); /*
							 * MANUAL gdx2d_pixmap* pixmap = gdx2d_new(width, height, format); if(pixmap==0)
							 * return 0;
							 * 
							 * jobject pixel_buffer = env->NewDirectByteBuffer((void*)pixmap->pixels,
							 * pixmap->width * pixmap->height * gdx2d_bytes_per_pixel(pixmap->format));
	...
	private static native void setPixel(long pixmap, int x, int y,
			int color); /*
						 * gdx2d_set_pixel((gdx2d_pixmap*)pixmap, x, y, color);
						 */

	private static native int getPixel(long pixmap, int x,
			int y); /*
					 * return gdx2d_get_pixel((gdx2d_pixmap*)pixmap, x, y);
					 */

	private static native void drawLine(long pixmap, int x, int y, int x2, int y2,
			int color); /*
						 * gdx2d_draw_line((gdx2d_pixmap*)pixmap, x, y, x2, y2, color);
						 */

	private static native void drawRect(long pixmap, int x, int y, int width, int height,
			int color); /*
						 * gdx2d_draw_rect((gdx2d_pixmap*)pixmap, x, y, width, height, color);
						 */

	private static native void drawCircle(long pixmap, int x, int y, int radius,
			int color); /*
						 * gdx2d_draw_circle((gdx2d_pixmap*)pixmap, x, y, radius, color);
						 */

	private static native void fillRect(long pixmap, int x, int y, int width, int height,
			int color); /*
						 * gdx2d_fill_rect((gdx2d_pixmap*)pixmap, x, y, width, height, color);
						 */

	private static native void fillCircle(long pixmap, int x, int y, int radius,
			int color); /*
						 * gdx2d_fill_circle((gdx2d_pixmap*)pixmap, x, y, radius, color);
						 */

	private static native void fillTriangle(long pixmap, int x1, int y1, int x2, int y2, int x3, int y3,
			int color); /*
	...
	private static native void drawPixmap(long src, long dst, int srcX, int srcY, int srcWidth, int srcHeight, int dstX,
			int dstY, int dstWidth, int dstHeight); /*
													 * gdx2d_draw_pixmap((gdx2d_pixmap*)src, (gdx2d_pixmap*)dst, srcX,
													 * srcY, srcWidth, srcHeight, dstX, dstY, dstWidth, dstHeight);
													 */
