 ...
 * keyboard, the touch screen and the accelerometer. On some backends (desktop,
 * gwt, etc) the touch screen is replaced by mouse input. The accelerometer is
 * of course not available on all backends.
 * </p>
 * 
 * <p>
 * Instead of polling for events, one can process all input events with an
 * {@link InputProcessor}. You can set the InputProcessor via the
 * {@link #setInputProcessor(InputProcessor)} method. It will be called before
 * the {@link ApplicationListener#render()} method in each frame.
 * </p>
 * 
 * <p>
 * Keyboard keys are translated to the constants in {@link Keys} transparently
 * on all systems. Do not use system specific key constants.
 * </p>
 * 
 * <p>
 * The class also offers methods to use (and test for the presence of) other
 * input systems like vibration, compass, on-screen keyboards, and cursor
 * capture. Support for simple input dialogs is also provided.
 * </p>
 * 
 * @author mzechner
 */
public interface Input {
	 ...
	 * {@link Input#getTextInput(TextInputListener, String, String, String)}
	 * 
	 * @author mzechner
	 */
...
// ? | VK_QUESTION
		...
		/**
		 * @return a human readable representation of the keycode. The returned value
		 *         can be used in {@link Input.Keys#valueOf(String)}
		 */
		public static String toString(int keycode) {
			...
			case F12:
				return "F12";
			// BUTTON_CIRCLE unhandled, as it conflicts with the more likely to be pressed
			// F12
			default:
				// key name not found
				return null;
	...
	/**
	 * @return The acceleration force in m/s^2 applied to the device in the X axis,
	 *         including the force of gravity
	 */
	public float getAccelerometerX();
	
	/**
	 * @return The acceleration force in m/s^2 applied to the device in the Y axis,
	 *         including the force of gravity
	 */
	public float getAccelerometerY();
	
	/**
	 * @return The acceleration force in m/s^2 applied to the device in the Z axis,
	 *         including the force of gravity
	 */
	...
	/**
	 * @return The x coordinate of the last touch on touch screen devices and the
	 *         current mouse position on desktop for the first pointer in screen
	 *         coordinates. The screen origin is the top left corner.
	 */
	public int getX();
	
	/**
	 * Returns the x coordinate in screen coordinates of the given pointer. Pointers
	 * are indexed from 0 to n. The pointer id identifies the order in which the
	 * fingers went down on the screen, e.g. 0 is the first finger, 1 is the second
	 * and so on. When two fingers are touched down and the first one is lifted the
	 * second one keeps its index. If another finger is placed on the touch screen
	 * the first free index will be used.
	 * 
	...
	/**
	 * @return the different between the current pointer location and the last
	 *         pointer location on the x-axis.
	 */
	...
	/**
	 * @return The y coordinate of the last touch on touch screen devices and the
	 *         current mouse position on desktop for the first pointer in screen
	 *         coordinates. The screen origin is the top left corner.
	 */
	public int getY();
	...
	/**
	 * Returns the y coordinate in screen coordinates of the given pointer. Pointers
	 * are indexed from 0 to n. The pointer id identifies the order in which the
	 * fingers went down on the screen, e.g. 0 is the first finger, 1 is the second
	 * and so on. When two fingers are touched down and the first one is lifted the
	 * second one keeps its index. If another finger is placed on the touch screen
	...
	/**
	 * @return the different between the current pointer location and the last
	 *         pointer location on the y-axis.
	 */
	 ...
	 * which the fingers went down on the screen, e.g. 0 is the first finger, 1 is
	 * the second and so on. When two fingers are touched down and the first one is
	 * lifted the second one keeps its index. If another finger is placed on the
	 * touch screen the first free index will be used.
	 * 
	...
	/**
	 * Whether a given button is pressed or not. Button constants can be found in
	 * {@link Buttons}. On Android only the Buttons#LEFT constant is meaningful
	 * before version 4.0.
	 * 
	 ...
	 * created with the given title and the given text as a message for the user.
	 * Once the dialog has been closed the provided {@link TextInputListener} will
	...
	/**
	 * Vibrate with a given pattern. Pass in an array of ints that are the times at
	 * which to turn on or off the vibrator. The first one is how long to wait
	 * before turning it on, and then after that it alternates. If you want to
	 * repeat, pass the index into the pattern at which to start the repeat.
	 * 
	 * @param pattern an array of longs of times to turn the vibrator on or off.
	 * @param repeat  the index into pattern at which to repeat, or -1 if you don't
	 *                want to repeat.
	 */
	 ...
	 * positive x-axis roughly points to the west and is orthogonal to the z- and
	 * y-axis.
	 * 
	 * @see <a
	 *      href="http://developer.android.com/reference/android/hardware/SensorManager.html#getRotationMatrix(float[], float[], float[], float[])">http://developer.android.com/reference/android/hardware/SensorManager.html#getRotationMatrix(float[],
	 ...
	 * float[], float[], float[])" >SensorManager#getRotationMatrix(float[],
	 * float[], float[], float[])</a>. Does not manipulate the matrix if the
	...
	/**
	 * @return the time of the event currently reported to the
	 *         {@link InputProcessor}.
	 */
	public long getCurrentEventTime();
	
	/**
	 * Sets whether the BACK button on Android should be caught. This will prevent
	 * the app from being paused. Will have no effect on the desktop.
	 * 
	...
	/**
	 * Sets whether the MENU button on Android should be caught. This will prevent
	 * the onscreen keyboard to show up. Will have no effect on the desktop.
	 * 
	...
	/**
	 * Sets the {@link InputProcessor} that will receive all touch and key input
	 * events. It will be called before the {@link ApplicationListener#render()}
	 * method each frame.
	 * 
	...
	/**
	 * Queries whether a {@link Peripheral} is currently available. In case of
	 * Android and the {@link Peripheral#HardwareKeyboard} this returns the whether
	 * the keyboard is currently slid out or not.
	 * 
	...
	/**
	 * Only viable on the desktop. Will confine the mouse cursor location to the
	 * window and hide the mouse cursor. X and y coordinates are still reported as
	 * if the mouse was not catched.
	 * 
	...
	/**
	 * Only viable on the desktop. Will set the mouse cursor location to the given
	 * window coordinates (origin top-left corner).
	 * 
