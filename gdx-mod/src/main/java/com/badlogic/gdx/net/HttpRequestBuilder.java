 ...
 * 
 * Make sure to call {@link #newRequest()} first, then set the request up and
 * obtain it via {@link #build()} when you are done.
 * 
 * It also offers a few utility methods to deal with content encoding and HTTP
 * headers.
 * 
...
public class HttpRequestBuilder {
	...
	/**
	 * Will be added as a prefix to each URL when {@link #url(String)} is called.
	 * Empty by default.
	 */
	public static String baseUrl = "";

	/**
	 * Will be set for each new HttpRequest. By default set to {@code 1000}. Can be
	 * overwritten via {@link #timeout(int)}.
	 */
	public static int defaultTimeout = 1000;

	/**
	 * Will be used for the object serialization in case
	 * {@link #jsonContent(Object)} is called.
	 */
	...
	public HttpRequestBuilder newRequest() {
		if (httpRequest != null) {
			throw new IllegalStateException(
					"A new request has already been started. Call HttpRequestBuilder.build() first.");
		}
	...
	/**
	 * The {@link #baseUrl} will automatically be added as a prefix to the given
	 * URL.
	 * 
	...
	/**
	 * Sets the correct {@code ContentType} and encodes the given parameter map,
	...
	/**
	 * Sets the correct {@code ContentType} and encodes the given content object via
	 * {@link #json}, then sets it as the content.
	 */
	...
	/**
	 * Sets the {@code Authorization} header via the Base64 encoded username and
	 * password.
	 */
	...
	/**
	 * Returns the {@link HttpRequest} that has been setup by this builder so far.
	 * After using the request, it should be returned to the pool via
	 * {@code Pools.free(request)}.
	 */
