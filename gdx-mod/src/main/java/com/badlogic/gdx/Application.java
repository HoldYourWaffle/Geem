 ...
 * <ul>
 * <li>a desktop application (see <code>JglfwApplication</code> found in
 * gdx-backends-jglfw.jar)</li>
 * <li>an Android application (see <code>AndroidApplication</code> found in
 * gdx-backends-android.jar)</li>
 * <li>a HTML5 application (see <code>GwtApplication</code> found in
 * gdx-backends-gwt.jar)</li>
 * <li>an iOS application (see <code>IOSApplication</code> found in
 * gdx-backends-robovm.jar)</li>
 * </ul>
 ...
 * 
 * <p>
 * While game programmers are used to having a main loop, libgdx employs a
 * different concept to accommodate the event based nature of Android
 * applications a little more. You application logic must be implemented in a
 * {@link ApplicationListener} which has methods that get called by the
 * Application when the application is created, resumed, paused, disposed or
 * rendered. As a developer you will simply implement the ApplicationListener
 * interface and fill in the functionality accordingly. The ApplicationListener
 ...
 * documentation of the Application implementations for more information. Note
 * that the ApplicationListener can be provided to any Application
 * implementation. This means that you only need to write your program logic
 * once and have it run on different platforms by passing it to a concrete
 ...
 * emulated via desktop OpenGL. On Android the functionality of the Java OpenGL
 * ES bindings is used.
 * </p>
 * 
 * <p>
 * {@link Audio} offers you various methods to output and record sound and
 * music. This is achieved via the Java Sound API on the desktop. On Android the
 * Android media framework is used.
 * </p>
 * 
 * <p>
 ...
 * implement an {@link InputProcessor} and use it with
 * {@link Input#setInputProcessor(InputProcessor)} to receive input events.
 * </p>
 * 
 * <p>
 * {@link Files} offers you various methods to access internal and external
 * files. An internal file is a file that is stored near your application. On
 * Android internal files are equivalent to assets. On the desktop the classpath
 * is first scanned for the specified file. If that fails then the root
 * directory of your application is used for a look up. External files are
 * resources you create in your application and write to an external storage. On
 * Android external files reside on the SD-card, on the desktop external files
 * are written to a users home directory. If you know what you are doing you can
 * also specify absolute file names. Absolute filenames are not portable, so
 * take great care when using this feature.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The <code>Application</code> also has a simple logging method which will
 * print to standard out on the desktop and to logcat on Android.
 * </p>
 * 
 * @author mzechner
 */
public interface Application {
	 ...
	 * will let all non-debug messages through, and {@link #LOG_DEBUG} will let all
	 * messages through.
	 * 
	 ...
	 * 
	 * In a multi-window application, the {@linkplain Gdx#graphics} and
	 * {@linkplain Gdx#input} values may be unpredictable at the time the Runnable
	 * is executed. If graphics or input are needed, they can be copied to a
	 * variable to be used in the Runnable. For example:
	 * <p>
	 ...
	 * {@link ApplicationListener} methods are sufficient for application level
	 * development.
	 * 
	 * @param listener
	 */
