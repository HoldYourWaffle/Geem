...
public class FreeType {
	// @off
	/*
	 * JNI #include <ft2build.h> #include FT_FREETYPE_H #include FT_STROKER_H
	 * 
	...
	public static class Library extends Pointer implements Disposable {
		...
		private static native long newMemoryFace(long library, ByteBuffer data, int dataSize,
				int faceIndex); /*
								 * FT_Face face = 0; FT_Error error = FT_New_Memory_Face((FT_Library)library,
								 * (const FT_Byte*)data, dataSize, faceIndex, &face); if(error) { lastError =
								 * error; return 0; } else return (jlong)face;
								 */
		...
		public Stroker createStroker() {
			...
			if (stroker == 0)
				throw new GdxRuntimeException(
						"Couldn't create FreeType stroker, FreeType error code: " + getLastErrorCode());
			return new Stroker(stroker);
		}
		
		private static native long strokerNew(
				long library); /*
								 * FT_Stroker stroker; FT_Error error = FT_Stroker_New((FT_Library)library,
								 * &stroker); if(error) { lastError = error; return 0; } else return
								 * (jlong)stroker;
								 */
	}
	...
	public static class Face extends Pointer implements Disposable {
		...
		private static native boolean selectSize(long face,
				int strike_index); /*
		...
		private static native boolean setCharSize(long face, int charWidth, int charHeight, int horzResolution,
				int vertResolution); /*
										 * return !FT_Set_Char_Size((FT_Face)face, charWidth, charHeight,
		...
		private static native boolean setPixelSizes(long face, int pixelWidth,
				int pixelHeight); /*
		...
		private static native boolean loadGlyph(long face, int glyphIndex,
				int loadFlags); /*
		...
		private static native boolean loadChar(long face, int charCode,
				int loadFlags); /*
		...
		private static native int getKerning(long face, int leftGlyph, int rightGlyph,
				int kernMode); /*
								 * FT_Vector kerning; FT_Error error = FT_Get_Kerning((FT_Face)face, leftGlyph,
								 * rightGlyph, kernMode, &kerning); if(error) return 0; return kerning.x;
								 */
		...
		private static native int getCharIndex(long face,
				int charCode); /*
	...
	public static class GlyphSlot extends Pointer {
		...
		private static native long getBitmap(
				long slot); /*
							 * FT_GlyphSlot glyph = ((FT_GlyphSlot)slot); return (jlong)&(glyph->bitmap);
							 */
		...
		private static native boolean renderGlyph(long slot,
				int renderMode); /*
		...
		private static native long getGlyph(
				long glyphSlot); /*
									 * FT_Glyph glyph; FT_Error error = FT_Get_Glyph((FT_GlyphSlot)glyphSlot,
									 * &glyph); if(error) { lastError = error; return 0; } else return (jlong)glyph;
									 */
	}
	...
	public static class Glyph extends Pointer implements Disposable {
		...
		private static native long strokeBorder(long glyph, long stroker,
				boolean inside); /*
									 * FT_Glyph border_glyph = (FT_Glyph)glyph; FT_Glyph_StrokeBorder(&border_glyph,
									 * (FT_Stroker)stroker, inside, 1); return (jlong)border_glyph;
									 */
		...
		private static native long toBitmap(long glyph,
				int renderMode); /*
									 * FT_Glyph bitmap = (FT_Glyph)glyph; FT_Error error =
									 * FT_Glyph_To_Bitmap(&bitmap, (FT_Render_Mode)renderMode, NULL, 1); if(error) {
									 * lastError = error; return 0; } return (jlong)bitmap;
									 */
		...
		private static native int getLeft(long glyph); /*
														 * FT_BitmapGlyph glyph_bitmap = ((FT_BitmapGlyph)glyph); return
														 * glyph_bitmap->left;
														 */
	...
	public static class Bitmap extends Pointer {
		...
		public ByteBuffer getBuffer() {
			if (getRows() == 0)
				// Issue #768 - CheckJNI frowns upon env->NewDirectByteBuffer with NULL buffer
				// or capacity 0
		...
		private static native ByteBuffer getBuffer(
				long bitmap); /*
								 * FT_Bitmap* bmp = (FT_Bitmap*)bitmap; return
								 * env->NewDirectByteBuffer((void*)bmp->buffer, bmp->rows * abs(bmp->pitch));
								 */
	...
	public static class GlyphMetrics extends Pointer {
		...
		private static native int getHoriBearingX(
				long metrics); /*
		...
		private static native int getHoriBearingY(
				long metrics); /*
		...
		private static native int getVertBearingX(
				long metrics); /*
		...
		private static native int getVertBearingY(
				long metrics); /*
	...
	public static class Stroker extends Pointer implements Disposable {
		...
		private static native void set(long stroker, int radius, int lineCap, int lineJoin,
				int miterLimit); /*
									 * FT_Stroker_Set((FT_Stroker)stroker, radius, (FT_Stroker_LineCap)lineCap,
									 * (FT_Stroker_LineJoin)lineJoin, miterLimit);
									 */
	...
	public static Library initFreeType() {
		...
		if (address == 0)
			throw new GdxRuntimeException(
					"Couldn't initialize FreeType library, FreeType error code: " + getLastErrorCode());
		else
			return new Library(address);
	}
	
	private static native long initFreeTypeJni(); /*
													 * FT_Library library = 0; FT_Error error =
													 * FT_Init_FreeType(&library); if(error) { lastError = error; return
													 * 0; } else return (jlong)library;
													 */
