...
/**
 * Generates {@link BitmapFont} and {@link BitmapFontData} instances from
 * TrueType, OTF, and other FreeType supported fonts.
 * </p>
 * 
 * Usage example:
 * 
 ...
 * </pre>
 * 
 * The generator has to be disposed once it is no longer used. The returned
 * {@link BitmapFont} instances are managed by the user and have to be disposed
 * as usual.
 * 
 * @author mzechner
 * @author Nathan Sweet
...
public class FreeTypeFontGenerator implements Disposable {
	...
	/**
	 * The maximum texture size allowed by generateData, when storing in a texture
	 * atlas. Multiple texture pages will be created if necessary. Default is 1024.
	 * 
	...
	/**
	 * Creates a new generator from the given font file. Uses
	 * {@link FileHandle#length()} to determine the file size. If the file length
	 * could not be determined (it was 0), an extra copy of the font bytes is
	 * performed. Throws a {@link GdxRuntimeException} if loading did not succeed.
	 */
		...
		try {
			if (fileSize == 0) {
				...
				byte[] data = StreamUtils.copyStreamToByteArray(input,
						fileSize > 0 ? (int) (fileSize * 1.5f) : 1024 * 16);
	...
	/**
	 * Generates a new {@link BitmapFont}. The size is expressed in pixels. Throws a
	 * GdxRuntimeException if the font could not be generated. Using big sizes might
	...
	public BitmapFont generateFont(FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
		...
		if (updateTextureRegions)
			parameter.packer.updateTextureRegions(data.regions, parameter.minFilter, parameter.magFilter,
	...
	/**
	 * Uses ascender and descender of font to calculate real height that makes all
	 * glyphs to fit in given pixel size. Source:
	...
	/**
	 * Uses max advance, ascender and descender of font to calculate real height
	 * that makes any n glyphs to fit in given pixel width.
	 * 
	...
	/**
	 * Uses max advance, ascender and descender of font to calculate real height
	 * that makes any n glyphs to fit in given pixel width and height.
	 * 
	 * @param width    the max width to fit (in pixels)
	...
	/**
	 * Generates a new {@link BitmapFontData} instance, expert usage only. Throws a
	 * GdxRuntimeException if something went wrong.
	 * 
	...
	Glyph createGlyph(char c, FreeTypeBitmapFontData data, FreeTypeFontParameter parameter, Stroker stroker,
		...
		// If a page was added, create a new texture region for the incrementally added
		// glyph.
	...
	/**
	 * Cleans up all resources of the generator. Call this if you no longer use the
	 * generator.
	 */
	@Override
	public void dispose() {
	...
	/**
	 * Sets the maximum size that will be used when generating texture atlases for
	 * glyphs with <tt>generateData()</tt>. The default is 1024. By specifying
	 * {@link #NO_MAXIMUM}, the texture atlas will scale as needed.
	 * 
	 * The power-of-two square texture size will be capped to the given
	 * <tt>texSize</tt>. It's recommended that a power-of-two value be used here.
	 * 
	 * Multiple pages may be used to fit all the generated glyphs. You can query the
	 * resulting number of pages by calling <tt>bitmapFont.getRegions().length</tt>
	 * or <tt>freeTypeBitmapFontData.getTextureRegions().length</tt>.
	 * 
	 ...
	 * memory, thus the {@link #getImagePaths()} and {@link #getFontFile()} methods
	 * will return null.
	 * 
	 * @author mzechner
	 * @author Nathan Sweet
	...
	static public class FreeTypeBitmapFontData extends BitmapFontData implements Disposable {
		 ...
		 * Glyph lastGlyph) { if (packer != null) packer.setPackToTexture(true); // All
		 * glyphs added after this are packed directly to the texture.
		 * super.getGlyphs(run, str, start, end, lastGlyph); super.getGly if (dirty) {
		 * dirty = false; packer.updateTextureRegions(regions, parameter.minFilter,
	...
	public static enum Hinting {
		...
		Full,
		/**
		 * Light hinting with fuzzy edges, but close to the original shape. Uses the
		...
		AutoMedium,
		/**
		 * Strong hinting with crisp edges at the expense of shape fidelity. Uses the
		 * FreeType auto-hinter.
		 */
		AutoFull,
	}
	 ...
	 * atlas. If no packer is specified, the generator will use its own PixmapPacker
	 * to pack the glyphs into a power-of-two sized texture, and the resulting
	 * {@link FreeTypeBitmapFontData} will have a valid {@link TextureRegion} which
	...
	public static class FreeTypeFontParameter {
		...
		/**
		 * Number of times to render the glyph. Useful with a shadow or border, so it
		 * doesn't show through the glyph.
		 */
		...
		/**
		 * Shadow color; only used if shadowOffset > 0. If alpha component is 0, no
		 * shadow is drawn but characters are still offset by shadowOffset.
		 */
		...
		/**
		 * The optional PixmapPacker to use for packing multiple fonts into a single
		 * texture.
		 * 
		...
		/**
		 * When true, glyphs are rendered on the fly to the font's glyph page textures
		 * as they are needed. The FreeTypeFontGenerator must not be disposed until the
		 * font is no longer needed. The FreeTypeBitmapFontData must be disposed
		 * (separately from the generator) when the font is no longer needed. The
		 * FreeTypeFontParameter should not be modified after creating a font. If a
		 * PixmapPacker is not specified, the font glyph page textures will use
